...
//!
//! These tests check the values returned by all the APIs in the `zakura_db::block` module,
...
//! ```sh
//! cargo insta test --review --release -p zakura-state --lib
//! ```
...
use crate::{
    service::{
        finalized_state::{
            ...
            zakura_db::transparent::BALANCE_BY_TRANSPARENT_ADDR,
            FinalizedState,
        },
...
fn snapshot_block_and_transaction_data(state: &FinalizedState) {
    ...
    if let Some((max_height, tip_block_hash)) = tip {
        ...
        let sprout_tree_at_tip = state
            .sprout_tree_for_tip()
            .expect("snapshot state has a Sprout tip tree");

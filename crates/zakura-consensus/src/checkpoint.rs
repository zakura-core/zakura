...
impl VerifyCheckpointError {
    ...
    pub fn body_verification_class(&self) -> zakura_header_chain::BodyVerificationClass {
        ...
        match self {
            ...
            // See the same arm in `block.rs`: a duplicated transaction can preserve
            // the Merkle root, so the body is the supplier's choice, not the header's.
    ...
    fn call(&mut self, block: Arc<Block>) -> Self::Future {
        ...
        let mut req_block = match self.queue_block(block.clone()) {
                ...
                if crate::block::commitment::is_padding_error(&error) =>
            {
                ...
                return async move {
                    crate::block::commitment::check_auth_bound_duplicates(state, &network, block)
                        .await?;
                    Err(error.into())
                }
                .boxed();

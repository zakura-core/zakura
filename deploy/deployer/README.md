...
The watchdog waits 30 minutes and posts one fleet alert when at least two
observable nodes share the same height and block hash. A missing or different
block hash keeps the 10-minute node alerts. Down alerts take precedence over
stalled alerts, so a node only produces one active alert at a time. The watchdog
also alerts if a dashboard endpoint is unreachable for at least 10 minutes. It
posts one recovery message when a node or dashboard recovers. Persistent
failures do not post on every poll cycle.

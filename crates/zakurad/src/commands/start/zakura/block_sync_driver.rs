...
fn native_optimistic_relay_allowed(
    ...
    policy == zakura_network::config::BlockRelayPolicy::Optimistic
    ...
    let outcome = match throughput_probe.as_ref() {
        ...
        None => {
            let relay = relay.filter(|relay| {
                native_optimistic_relay_allowed(
                    relay.policy,
                    class,
...
mod tests {
    ...
    #[test]
    fn native_optimistic_relay_requires_a_full_tip_child_near_the_tip() {
        use zakura_network::config::BlockRelayPolicy::{Committed, Optimistic};

        let tip = block::Hash([3; 32]);
        let side_parent = block::Hash([4; 32]);
        assert!(native_optimistic_relay_allowed(
            Optimistic,
            BlockApplyClass::Full,
            true,
            Some(tip),

//! Basic integration tests for zakura-state
...
> = Lazy::new(|| {
    ...
    vec![
        (
            Request::CheckBlockCommitment(BlockCommitmentData {
                block: block.clone(),
                auth_data_root: None,
            }),
...
> = Lazy::new(|| {
    let block: Arc<_> =
        Block::zcash_deserialize(&zakura_test::vectors::BLOCK_TESTNET_GENESIS_BYTES[..])
            .unwrap()
            .into();
    let block2 = block.clone();
    let hash = block.hash();

...
Zakura relies on numerous dependencies maintained by ZODL, and updating them can be a complex task. This guide will help you navigate the process.

The main dependency that influences that is [zcash](https://github.com/zcash/zcash) itself. This is because [zakura_script](https://github.com/ZcashFoundation/zcash_script) links to specific files from it (zcash_script.cpp and all on which it depends). Due to the architecture of zcash, this requires linking to a lot of seemingly unrelated dependencies like orchard, halo2, etc (which are all Rust crates).
...
- Upgrade and release `zcash_script` before upgrading other ECC dependencies in Zakura.
...
- Ensure that the crate versions in the `Cargo.toml` of the zcashd release, `Cargo.toml` of `zcash_script`, and the `Cargo.toml` files of Zakura crates are all the same. Version consistency is crucial.

### Build/Test zakura & fix issues

- Build zakura and make sure it compiles.

```sh
cargo build
```

- Test Zakura and make sure all test code compiles and all tests pass:
...
- Review Zakura's `deny.toml` file for potential duplicates that can be removed due to the upgrade. You may also need to add new entries to `deny.toml`.

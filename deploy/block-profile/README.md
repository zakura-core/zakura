...
The CPU page defaults to recorded context followed by native frames. The stage selector and CPU links in the waterfall include that stage and its recorded descendants. Raw stacks preserve the original symbols. Unassigned CPU includes work without exclusive context, including shared verification batches and currently uninstrumented parallel iterations. No whole batch is charged to one transaction. Execution context is suppressed while Rayon can run unrelated stolen work. This is incremental instrumentation, not complete causal tracing of every task. The clock uncertainty margin is excluded at each association boundary. CPU-period weights remain statistical estimates, not exact per-stage accounting. Old recordings are never backfilled from completion-thread IDs.
...
Waterfall CPU links appear only when retained samples match that stage or its descendants. Counts load separately from the timeline. Pending or unavailable captures show no stage links until the page is refreshed. Short CPU jobs can fall between samples, and transaction elapsed time includes waits. Shared proof batches remain unassigned.

...
- **`zakura-vct-handoff-canary.yml`** — daily, manual, reusable, and release-state-PR Mainnet test that forces the Zakura P2P `tree_aux` path from the pinned pre-checkpoint state and exits after C is finalized and the best chain reaches C+1 with VCT fast-path activity. Failures alert `#zakura-alerts`; release-state PRs require the crossing to succeed, while normal releases dispatch it as an advisory check.

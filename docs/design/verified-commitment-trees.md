...
Databases written by the original VCT implementation can be missing historical Sprout
frontiers because that implementation returned from the fast write path before updating
Sprout. They cannot safely validate later JoinSplit spends against those missing anchors.
Operators must discard and re-sync an affected existing VCT database in pruning mode; there is
no startup repair or format migration for this issue. Fresh VCT syncs persist every changed
Sprout `root → frontier` entry.
...
- reads the Sprout tip tree, which must correspond to the requested height;

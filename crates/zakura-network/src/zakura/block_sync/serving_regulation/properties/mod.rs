...
fn replay(scenario: &Scenario) -> Result<Vec<Observation>, String> {
    replay_with_writer(scenario, false)
}

fn replay_with_writer(
    scenario: &Scenario,
    drop_write_guard: bool,
    ...
    runtime.block_on(replay_actions(scenario, drop_write_guard))
}

async fn replay_actions(
    scenario: &Scenario,
    drop_write_guard: bool,
) -> Result<Vec<Observation>, String> {
    let mut model = Model::new(scenario.limit, block_payload_bytes());
    let mut production = Production::new(scenario.limit, fixture());
    production.drop_write_guard = drop_write_guard;

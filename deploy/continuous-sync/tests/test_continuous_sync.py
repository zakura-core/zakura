    ...
    def test_final_ready_sample_supplies_confirmed_height_without_stale_fallback(self):
        for final_height in (101, None, -1, True, 2**32):
            with self.subTest(final_height=final_height), tempfile.TemporaryDirectory() as tmp:
                ...
                run_dir = Path(tmp) / "run"
                run_dir.mkdir()
                early = {**exact_sync_sample(100, 100), "ready": True,
                         "zcash_chain_verified_block_height": 100}
                final = {**exact_sync_sample(105, 105), "ready": True,
                ...
                # Progress follows the committed height; throughput follows only
                # the final verified-height gauge.

...
Integration tests for zakurad's **zcashd-compat mode**, in which zakurad acts as
...
The test harness spawns a fresh zakurad and zcashd on randomised ports, runs
...
1. zakurad must be running with zcashd-compat enabled and fully synced to the
   network tip.

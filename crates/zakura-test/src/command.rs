...
/// Wrappers for `Command` methods to integrate with [`zakura_test`](crate).
...
///
/// This trait is separate from `ZakuradTestDirExt`, so that we can test
/// `zakura_test::command` without running `zakurad`.
...
impl<T> TestChild<T> {
    ...
    pub fn wait_with_output(mut self) -> Result<TestOutput<T>> {
        ...
        // TODO: fix the usage in the zakurad acceptance tests, or fix the bugs in TestChild,

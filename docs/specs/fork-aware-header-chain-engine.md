...
A later known body-size hint for the same semantic payload MAY update scheduling metadata without adding an evidence slot. Unknown MUST NOT erase known. The original delivery and its verification provenance MUST remain unchanged. Corrections MUST be bounded to one per retained delivery and pruned with it. An ordinary header batch with auxiliary deliveries MUST bind replay identity to both the prepared batch and the delivery attempt, so a new supplier can correct scheduling metadata without weakening conflicting-replay checks within an attempt.

...
- Embedded published completed subtree roots alongside the Mainnet last checkpoint, letting a
  verified-commitment-trees fast-synced node serve `z_getsubtreesbyindex` below that last
  checkpoint. Published roots never displace the node's own rows
...
- Added the `export-historical-treestates` subcommand, which generates the artifact from an
  archive state database, and `verify-historical-treestates`, which proves one against a frontier
  without a state database ([#593](https://github.com/zakura-core/zakura/pull/593)).

...
- A floor request with a fresh delivery-rate sample MUST use `floor_rescue_timeout`
  ...
  height MUST return to the queue. If its request write started, the peer MUST
  be retry-avoided — rescued, not disconnected (record-only).
...
If a request expires before its write starts, it MUST be skipped and its work

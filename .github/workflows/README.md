...
- **`create-release.yml`** — the only supported path for creating `v*` release tags. For a normal release, it dispatches the advisory Mainnet VCT handoff canary and calls `release-binaries.yml` to build and verify every asset before requesting protected-environment approval. It also dispatches `release-start-canary.yml` for non-source-first hotfixes, without waiting for it. Emergency source-first releases skip the pre-tag canary and asset build. See the release runbook before using it.
- **`release-start-canary.yml`** — after a same-repository `A-release` PR from `release/v*` merges to `main`, deploys that exact merged commit to `us-east-0` and verifies that the service remains active with working RPC and no restarts. The independent advisory run does not delay the `Create release` approval; a failed deploy restores the previous binary before reporting. It is also manually dispatchable for an exact commit, which is how `create-release.yml` covers hotfix branches.

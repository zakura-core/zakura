...
| `analysis/zakura_trace_analysis/analyze_zakura_sync.py` + `.agents/skills/zakura-trace-plots` | Post-hoc bottleneck attribution from JSONL traces (blocking-class classification: `hol_gap`, `commit_backpressure`, `download_starved`, …) — deeper drill-down behind the bench's verdict JSON. |

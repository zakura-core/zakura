...
use zakura_chain::{transaction, transparent};
        ...
        for mined_tx_id in mined_ids {
            for dependent_id in self.dependents.remove(mined_tx_id).unwrap_or_default() {
                let Some(dependencies) = self.dependencies.get_mut(&dependent_id) else {
                    // TODO: Move this struct to zakura-chain and log a warning here.
                    continue;
                };

                // TODO: Move this struct to zakura-chain and log a warning here if the dependency was not found.

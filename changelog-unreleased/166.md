...
- Bound the stream-reopen loops against unresponsive peers: a block-sync stream
  EOF with unanswered requests now parks or disconnects the peer like a
  liveness stall, discovery sessions retire after repeated failed exchanges
  instead of reopening forever, legacy-gossip reopen churn from zero-frame
  sessions is capped, and header-sync reopen waits target the reactor's real
  advisory-backoff expiry
  ([#166](https://github.com/zakura-core/zakura/pull/166)).

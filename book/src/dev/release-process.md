...
Every release tag must match the `zakura` package version: release binaries are built without `.git`, so `zakurad --version` reports the package version, not the tag. Create tags using the protected `create-release.yml` workflow, which verifies the package version, builds the assets from the validated commit, and creates the immutable tag only when publishing the complete pre-release. See [Protected Release Tags](https://github.com/zakura-core/zakura/blob/main/docs/release-tag-protection.md) for configuration and operation details.

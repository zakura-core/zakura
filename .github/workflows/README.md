...
- **`create-release.yml`** — the only supported path for creating `v*` release tags. It first resolves release preparation: if `main` already carries the tag's version the step is a no-op, otherwise `auto_prepare` (on by default) calls `prepare-release-pr.yml` with `auto_merge`, which prepares the release and squash-merges the PR unattended, and the release is then tagged at that merge commit. Before a normal release, it requires the Mainnet VCT handoff canary and calls `release-binaries.yml` to build and verify every asset; then a protected environment lets the release GitHub App publish the draft and create the immutable tag. Emergency source-first releases skip both pre-tag gates and require an already-prepared `main`. See the release runbook before using it.
...
- **`prepare-release-pr.yml`** — manual, and callable from `create-release.yml`: mechanically prepares a release PR for a given tag (crate bumps chosen by `cargo-semver-checks` against the base tag, zakura version, lockfile, config fixture, end-of-support floor, changelog assembly via `scripts/prepare-release.sh`), verifies with `make pre-release`, and opens a PR through the release GitHub App. By default the PR is a draft and the judgment items (bump-level review, authoritative end-of-support height, changelog curation) stay with the reviewer; a `dry_run` input uploads the diff as an artifact instead of opening a PR. With `auto_merge` the verification steps become a hard gate and the App squash-merges the PR unattended, which needs the release App on the `main` ruleset bypass list (pull requests only) — see `docs/release-tag-protection.md`.

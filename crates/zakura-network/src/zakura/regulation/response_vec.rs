//! Keep a collection's allocated storage charged even when it becomes empty.
//!
...
use super::{collection_allocation_bytes, response_memory::ResponseMemoryPermit, MemoryFull};

/// A vector whose capacity has a memory permit. Heap allocations inside its
/// entries need separate permits with their own lifetimes.
#[derive(Debug)]
...
// Start small collections with room for a few entries to avoid reallocating on
// each insertion. Exact growth remains available when the budget is tight.
const INITIAL_GROWTH_CAPACITY: usize = 4;
...
impl<T> ResponseVec<T> {
    ...
    pub(crate) fn plan_capacity(
        ...
        additional: usize,
        geometric: bool,
    ) -> Result<Option<CapacityPlan<T>>, MemoryFull> {
        let required = self.len().checked_add(additional).ok_or(MemoryFull)?;
        ...
        let capacity = if geometric {
            required
                ...
                .max(INITIAL_GROWTH_CAPACITY)
        } else {
            required
        };
        let bytes = collection_allocation_bytes::<T>(capacity).ok_or(MemoryFull)?;

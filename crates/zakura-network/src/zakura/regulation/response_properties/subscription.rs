//! Check a future message that keeps sending updates until it is closed.
//! This test adapter reuses production credit and lifetime rules. It has its own
//! policy for updates already in flight when Close is sent. Header v8 still uses
//! GetHeaders, so these tests do not claim a production subscription exists.

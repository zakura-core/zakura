...
impl PeerRoutine {
    ...
    async fn handle_frame(
        ...
        let body_permit = if frame_type == MSG_BS_BLOCK {
            ...
            let header = match self.decoder.decode::<block::Header>(
                &mut &frame.payload[super::wire::BLOCK_SYNC_MESSAGE_TYPE_BYTES..],
            ) {
                Ok(header) => header,
                Err(error) => {
                    self.report_misbehavior(BlockSyncMisbehavior::MalformedMessage)
        ...
        #[cfg(test)]
        let decoded = response_contract::observe_decode(&self.decode_probe, self.decoder, frame);
        #[cfg(not(test))]
        let decoded = BlockSyncMessage::decode_frame_with_raw_block_payload(frame, self.decoder);
    ...
    async fn try_fill(&mut self) -> Option<Instant> {
        ...
        let fill_stop: FillStop = loop {
            ...
            // expires (see `earliest_deadline_sleep`).
            {
                ...
                if !returned_avoided.is_empty() {
                    if let Some(deadline) = self.retry_filter_wake_deadline(
                        now,
                        returned_avoided.iter().map(|(height, _)| *height),
                    ) {

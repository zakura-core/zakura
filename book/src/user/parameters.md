...
These parameters are built into the `zakurad` binary. They are predefined keys that will allow verification of the circuits. They were initially obtained by [this process](https://eprint.iacr.org/2017/1050.pdf).
...
Zakura uses the [bellman crate groth16 implementation](https://crates.io/crates/groth16) for all groth16 types.

Each time a transaction has any sprout joinsplit, sapling spend or sapling output these loaded parameters will be used for the verification process. Zakura verifies in parallel and by batches, these parameters are used on each verification done.

The first time any parameters are used, Zakura automatically parses all of the parameters. This work
is only done once.

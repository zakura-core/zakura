...
use crate::zakura::{
    ...
    FramedRecv, FramedSend, Service, ServicePeerSnapshot, ServiceRegistry, SessionDemand,
...
pub(super) fn mainnet_decoder() -> zakura_chain::serialization::ZcashDecoder {
...
fn raw_block_payload(block: &Arc<block::Block>) -> super::wire::RawBlockPayload {
    ...
    BlockSyncMessage::decode_frame_with_raw_block_payload(frame, mainnet_decoder())
        .unwrap()
        .1
        .unwrap()
}
...
fn block_liveness_disconnects_silent_active_peer_after_default_timeout() {
    ...
    window.push_outstanding_for_test(window_request(1));
...
fn block_liveness_resuming_after_idle_gets_fresh_deadline() {
    ...
    window.push_outstanding_for_test(window_request(2));
...
fn block_liveness_multi_block_range_progress_resets_each_body() {
    ...
    let mut window = download_window();
    window.push_outstanding_for_test(window_request_range(1, 3));
...
async fn peer_emits_periodic_bbr_heartbeat_while_idle() {
    ...
    let service = BlockSyncService::new_with_handle(config, _handle.clone(), mainnet_decoder());
...
async fn service_registry_routes_block_sync_by_exact_capability_and_version() {
    ...
    assert!(registry
        .persistent_streams_for_negotiated(1 << 3)
        .is_empty());
    assert!(registry
        .capability_for_stream(ZAKURA_STREAM_BLOCK_SYNC, 2)
        .is_none());
    assert_eq!(
        registry.persistent_streams_for_negotiated((1 << 3) | ZAKURA_CAP_BLOCK_SYNC),
        registry.persistent_streams_for_negotiated(ZAKURA_CAP_BLOCK_SYNC),
...
async fn inert_reactor_parks_after_header_tip_watch_closes() {
    let _service = BlockSyncService::new(ZakuraBlockSyncConfig::default(), mainnet_decoder());
...
async fn reconciliation_rejects_stale_admissions_and_service_ignores_stale_disconnects() {
    ...
    let service =
        BlockSyncService::new_with_handle(config.clone(), handle.clone(), mainnet_decoder());
...
#[tokio::test]
async fn network_bound_peer_decode_rejects_the_other_networks_block() {
    use zakura_chain::{
        parameters::Network, serialization::ZcashDecoder, work::equihash::Solution,
    };
...
async fn assert_peer_decode_failure(
    ...
    let (handle, mut actions, reactor_task) = spawn_block_sync_reactor(startup);
    let service = BlockSyncService::new_with_handle(config, handle.clone(), decoder);
...
async fn reactor_discards_duplicate_buffered_body_and_keeps_first_receipt() {
    ...
    // is dropped unread. The ending closes that range and returns the spent
    // height 1 to `pending`.
    send_inbound(&inbound_tx, BlockSyncMessage::Block(blocks[1].clone())).await;
    send_inbound(&inbound_tx, BlockSyncMessage::Block(blocks[1].clone())).await;
    send_inbound(
    ...
    // The duplicate is neither misconduct nor a commit, and the buffered
    // successor keeps waiting for its parent.
    let quiet = tokio::time::timeout(Duration::from_millis(250), async {

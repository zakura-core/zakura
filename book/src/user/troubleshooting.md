...
There are a few bugs in Zakura that we're still working on fixing:
...
- If Zakura's build runs out of RAM, try setting `export CARGO_BUILD_JOBS=2`.
- If Zakura's tests timeout or run out of RAM, try running `cargo test -- --test-threads=2`. Note that `cargo` uses all processor cores on your machine
  by default.

## Network Issues

Some of Zakura's tests download Zcash blocks, so they might be unreliable
depending on your network connection. You can set `SKIP_NETWORK_TESTS=1`
to skip the network tests.

## Issues with Tests on macOS

Some of Zakura's tests deliberately cause errors that make Zakura panic. macOS
records these panics as crash reports. If you are seeing "Crash Reporter"
dialogs during Zakura tests, you can disable them using this Terminal.app
command:
...
Zakura typically syncs in around three days on Mainnet and half a day on
Testnet under optimal conditions. The actual sync speed depends on your network
connection, hardware, and the overall Zcash network load. The major constraint we've found on `zakurad` performance is the
...
If your connection is slow, try [connecting to fewer peers](https://docs.rs/zakura-network/latest/zakura_network/config/struct.Config.html#structfield.peerset_initial_target_size):
...
Zakura logs at info level by default.

If Zakura is slow, make sure it is logging at info level:
...
Or restrict debug logging to a specific Zakura component:
...
[open a bug.](https://github.com/zakura-core/zakura/issues/new/choose)
...
> connection on the host and every program using it, not just Zakura. Review
...
`/etc/sysctl.d/99-zakura-network.conf` and reload with
`sudo sysctl --system`. To apply for the current boot only, use
`sudo sysctl -w <key>=<value>` for each line.

Zakura logs a warning at startup on Linux if `tcp_slow_start_after_idle` is
enabled.

#### Running Zakura in Docker

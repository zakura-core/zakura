...
fn adjust_difficulty_and_time_for_testnet(
    ...
    // A minimum-difficulty block is only valid when its time is strictly greater than
    // `previous_block_time + 6 * PoWTargetSpacing`. Do not future-date templates just to reach
    // that threshold, because each artificial minimum-difficulty block depresses the Testnet
    // difficulty average.
...
mod tests {
    ...
    #[test]
    fn testnet_template_is_not_future_dated_before_minimum_difficulty_threshold() {
        let network = Network::new_default_testnet();
        let current_time = PREVIOUS_BLOCK_TIME + 400;
        let threshold = network.target_difficulty_limit().to_compact();
        let recent_block_data = (0..POW_ADJUSTMENT_BLOCK_SPAN)
            .map(|offset| {
                let offset = u32::try_from(offset)
                    .expect("the difficulty adjustment window always fits in u32");
                (
                    threshold,
        ...
        let mut chain_info = GetBlockTemplateChainInfo {
            ...
            cur_time: DateTime32::from(current_time),
        ...
        assert_eq!(chain_info.expected_difficulty, CompactDifficulty::default());
        assert_eq!(chain_info.cur_time, DateTime32::from(current_time));
        assert_eq!(
            chain_info.max_time,
            DateTime32::from(PREVIOUS_BLOCK_TIME + 6 * 75)
        );
    }
}

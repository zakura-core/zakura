...
use zakura_state::{BoxError, GetBlockTemplateChainInfo};
...
proptest! {
            ...
            let mock_state_handler = {
                ...
                async move {
                    ...
                    state
                        ...
                        .await
                        .expect("getblockchaininfo should call mock state service with correct request")
                        .respond(zakura_state::ReadResponse::TipPoolValues {
    ...
    fn get_blockchain_info_returns_genesis_when_tip_pool_fails(network in any::<Network>()) {
        ...
        let genesis_block = match network {
            Network::Mainnet => {
                let block_bytes = &zakura_test::vectors::BLOCK_MAINNET_GENESIS_BYTES;
                let block: Arc<Block> = block_bytes.zcash_deserialize_into().expect("block is valid");
                block
            },
            Network::Testnet(_) => {
                let block_bytes = &zakura_test::vectors::BLOCK_TESTNET_GENESIS_BYTES;
                let block: Arc<Block> = block_bytes.zcash_deserialize_into().expect("block is valid");
                block
            },
        };
    ...
    fn queries_balance_for_valid_addresses(
        ...
        runtime.block_on(async move {
            ...
            let state_query = state
                .expect_request(zakura_state::ReadRequest::AddressBalance(addresses))
                .map_ok(|responder| {
                    responder.respond(zakura_state::ReadResponse::AddressBalance { balance, received: balance.into() })
                });
    ...
    fn rpc_queue_main_loop(tx in any::<Transaction>(), network in any::<Network>()) {
        ...
        runtime.block_on(async move {
            ...
            let expected_request = zakura_state::ReadRequest::Transaction(transaction_hash);
            let response = zakura_state::ReadResponse::Transaction(None);
            ...
            for _tx in txs.clone() {
                ...
                state
                    .expect_request_that(|request| {
                        matches!(request, zakura_state::ReadRequest::Transaction(_))
                    })
                    .await?
                    .respond(response);
            }
...
fn mock_services<Tip>(
    ...
    zakura_test::mock_service::MockService<

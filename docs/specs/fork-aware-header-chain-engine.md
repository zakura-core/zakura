...
**LC-BODY-01 [ZW] — Body-payload mismatch.** If a delivered body has a different header hash, fails to reproduce the requested header’s transaction Merkle root, ZIP 244 authorization-data commitment, or other applicable body-derived header commitment, the engine MUST classify that delivery as `BodyPayloadMismatch`, attribute it only to the body supplier, discard it, and retry another source without changing header eligibility, unless independently committed fields already prove intrinsic invalidity under LC-BODY-02.

**LC-BODY-02 [ZW] — Consensus-invalid body feedback.** If the header commitments bind every field needed to prove a deterministic body or state consensus failure, the engine MUST durably add `ConsensusBodyInvalid(evidence)` to that block and propagate body-ineligibility to its descendants. It MUST then independently rerun header selection. Header suppliers MUST NOT be blamed for this result.

For example, a transaction expiry height fixed by a matching transaction Merkle root can prove invalidity even when unrelated NU5 authorization bytes do not match. A NU5 signature or proof failure requires a matching authorization commitment checked against the exact parent's history. Missing history is retryable. Duplicated transactions alone do not prove intrinsic invalidity because Merkle padding can preserve the root. A reconstructed unique candidate must use the shared consensus rules and actual parent state, including side branches, without committing or caching the reconstruction.

...
///
/// The first block is a pre-Heartwood block so committing the fake children
/// doesn't trigger history tree updates in the non-finalized state.
...
async fn snapshots_carry_known_tip_changes() {
    ...
    let mut state = NonFinalizedState::new(&network);
    let root = blocks[0].clone().prepare();
    state.commit_new_chain(root, &finalized).unwrap();
    let child = blocks[1].clone().prepare();
...
async fn skips_blocks_at_or_below_known_tip() {
    ...
    // The caller already has block2 and its ancestor block1, so only block3 is
    // new.
...
/// When the caller has the current best tip, nothing is sent until the state
/// changes.
#[tokio::test]
...
/// After the initial send, only blocks added since the previous state are
/// forwarded; known tips don't re-filter later updates.
#[tokio::test]

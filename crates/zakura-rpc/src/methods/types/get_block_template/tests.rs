...
fn coinbase_cache_detaches_when_miner_data_changes() {
    ...
    let fee = Amount::zero();
    let original =
        TransactionTemplate::new_coinbase(&net, height, handler.miner_params().unwrap(), fee)
            .unwrap();
    ...
    let replacement =
        TransactionTemplate::new_coinbase(&net, height, randomized.miner_params().unwrap(), fee)
            .unwrap();

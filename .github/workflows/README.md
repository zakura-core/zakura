...
| `codex-approval-adapter.yml` | Converts a current, clean native Codex review into an approval for eligible CI/deploy changes. Defaults to a read-only audit; release automation requires human review. See the [approval policy](../review-policy/README.md). | PR metadata/comments; policy changes; hourly reconciliation; manual |
...
- **`prepare-release-pr.yml`** — manual: mechanically prepares a release PR for a given tag (crate bumps chosen by `cargo-semver-checks` against the base tag, zakura version, lockfile, config fixture, end-of-support floor, changelog assembly via `scripts/prepare-release.sh`), verifies with `make pre-release`, and opens a draft PR through the release GitHub App. Judgment items (bump-level review, end-of-support halt height via `#zakura-collab` or Dev/Sean, changelog curation) stay with the reviewer and are gated by `release-pr-readiness.yml`; a `dry_run` input uploads the diff as an artifact instead of opening a PR.
...
Runtime regions are tried in order (`nyc1,sfo3,nyc3`). Only regions with an available compatible image and the requested state are eligible. An explicit artifact ID is never substituted. Pre-checkpoint selection requires a known height strictly below C; the restored database is still checked independently before networking starts. Older ordinary snapshots without the `-finalized-h<height>` marker remain usable for tip and sandblast tests, but are excluded from automatic and exact-ID handoff selection. Dedicated approach fixtures remain eligible and are checked again on restore. Retention uses the same eligibility rules so an old publisher-tip label cannot displace a verified fixture. After merging a bake change, validate the new regional artifacts by exact ID and run the current-checkpoint canary; a successful bake alone does not prove a handoff.

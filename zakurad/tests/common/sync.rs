//! `zakurad` sync-specific shared code for the `zakurad` acceptance tests.
//!
//! # Warning
//!
...
use zakurad::{components::sync, config::ZakuradConfig};

use zakura_test::{args, prelude::*};
...
pub const MEDIUM_CHECKPOINT_TEST_HEIGHT: Height =
    Height(zakura_consensus::MAX_CHECKPOINT_HEIGHT_GAP as u32);
pub const LARGE_CHECKPOINT_TEST_HEIGHT: Height =
    Height((zakura_consensus::MAX_CHECKPOINT_HEIGHT_GAP * 2) as u32);
...
/// The maximum time to wait for Zakurad to synchronize up to the chain tip starting from a
...
/// The maximum time to wait for Zakurad to synchronize up to the chain tip starting from the
/// genesis block.
...
pub enum MempoolBehavior {
    ...
    ///
    /// [`sync_until`] will kill `zakurad` after it logs mempool activation,
    /// then the `stop_regex`.
    ...
    /// # Correctness
    ///
    /// Unlike the other mempool behaviours, `zakurad` must stop after logging the stop regex,
    /// without being killed by [`sync_until`] test harness.
    ///
    /// Since it needs to collect all the output,
    /// the test harness can't kill `zakurad` after it logs the `stop_regex`.
    ShouldNotActivate,
}
...
/// Sync on `network` until `zakurad` reaches `height`, or until it logs `stop_regex`.
///
...
///
/// Configures `zakurad` to debug-enable the mempool based on `mempool_behavior`,
/// then check the logs for the expected `mempool_behavior`.
///
/// If `checkpoint_sync` is true, configures `zakurad` to use as many checkpoints as possible.
...
pub fn sync_until(
    ...
    if mempool_behavior.require_activation() {
        ...
        // checking the entire `zakurad` output after it exits.
        //
        // # Correctness
        //
        // Unlike the other mempool behaviours, `zakurad` must stop after logging the stop regex,
        ...
        assert!(
            height.0 < 2_000_000,
            "zakurad must exit by itself, so we can collect all the output",
        );
...
/// Check sync logs on `network` until `zakurad` logs `stop_regex`.
///
/// ## Test Settings
///
...
/// ## Test Status
///
/// Returns the provided `zakurad` [`TestChild`] when `stop_regex` is encountered.
///
/// Returns an error if the child exits or `timeout` elapses before `stop_regex` is found.
#[tracing::instrument(skip(zakurad))]
pub fn check_sync_logs_until(
    mut zakurad: TestChild<TempDir>,
    network: &Network,
    stop_regex: &str,
    // Test Settings
    mempool_behavior: MempoolBehavior,
    ...
    if check_legacy_chain {
        ...
        zakurad.expect_stdout_line_matches("no legacy chain found")?;

        zakurad.expect_stdout_line_matches("starting state checkpoint validation")?;
        // TODO: what if the mempool is enabled for debugging before this finishes?
        zakurad.expect_stdout_line_matches("finished state checkpoint validation")?;
    }

    // before the stop regex, expect mempool activation
    if mempool_behavior.require_forced_activation() {
        zakurad.expect_stdout_line_matches("enabling mempool for debugging")?;
    }
    ...
    zakurad.expect_stdout_line_matches(stop_regex)?;

    Ok(zakurad)
}

/// Returns the cache directory for Zebra's state, as configured with [`ZakuradConfig::load`] with config-rs.
///
...
/// If `check_legacy_chain` is true, make sure the logs contain the legacy chain check.
///
/// The test passes when `zakurad` logs the `stop_regex`.
...
pub fn create_cached_database_height(
    ...
    height: Height,
    checkpoint_sync: bool,
    storage_mode: zakura_state::StorageMode,
    stop_regex: &str,
) -> Result<()> {

...
use zakura_chain::{parameters::Network, serialization::ZcashSerialize};
...
pub(crate) async fn run() -> Result<()> {
    ...
    tracing::info!(
        ?network,
        ?test_type,
        "running getblocktemplate test using zakurad",
    );

    let should_sync = true;
    let (zakurad, zakura_rpc_address) =
        spawn_zakurad_for_rpc(network.clone(), test_name, test_type, should_sync)?
            .ok_or_else(|| eyre!("getblocktemplate test requires a cached state"))?;

    let rpc_address = zakura_rpc_address.expect("test type must have RPC port");

    let mut zakurad = check_sync_logs_until(
        zakurad,
        &network,
        SYNC_FINISHED_REGEX,

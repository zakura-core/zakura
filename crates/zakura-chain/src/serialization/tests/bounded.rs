...
fn counted_collections_reject_missing_input_before_decoding_elements() {
    ...
    for remaining in [0, 1, 31, 32, 63] {
        ...
        let (result, allocated) =
            zakura_test::allocations::measure(|| reader.read_external_count::<Hash>(2));
        assert!(matches!(
            result,
            Err(SerializationError::Parse("Vector exceeds available input"))
        ));
        assert_eq!(allocated.peak_live_bytes, 0);
...
#[test]
fn nested_byte_strings_reject_counts_larger_than_the_input() {
...
#[test]
fn slice_and_stream_decoding_agree_at_chunk_boundaries() {

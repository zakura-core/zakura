...
A body matches all applicable commitments, and is fully bound, when its transaction list reproduces the header's Merkle root without duplicates that Merkle padding could have introduced and, from NU5, its ZIP 244 authorization-data commitment computed from the delivered bytes matches the header against the exact parent's history. A duplicate list whose duplicates cannot be removed while preserving the root, or that the NU5 authorization commitment proves the header commits to, is fully bound and intrinsically invalid. The engine MUST NOT record `ConsensusBodyInvalid` from a body that is not fully bound, even when fields the header does commit to would prove the failure on their own: such a delivery is a `BodyPayloadMismatch`, and the engine does not reconstruct or validate candidate bodies. A header whose delivered bodies never bind is handled by LC-AVAIL-01 and LC-AVAIL-02 exactly like a header whose body is unavailable, which bounds the cost of a proof-of-work-backed invalid header to the same episode as a withheld body. Missing parent history is retryable under LC-BODY-03.

...
- **`zakura-vct-handoff-canary.yml`** — daily, manual, and reusable Mainnet test that forces the Zakura P2P `tree_aux` path from the pinned pre-checkpoint state and exits as soon as finalized state crosses the VCT handoff. Failures alert `#zakura-alerts`; normal releases call it before publishing.

...
def read_capture_metrics(metrics, families):
    """Read selected counter families; keep label order out of their identity.

    Values must be exact nonnegative integers within the exporter's float range.
    Unrelated families are ignored, including unrelated capture diagnostics.
    """
    try:
    ...
    samples = {}
    names = {"sync_block_capture_" + family for family in families}
    for line in lines:
        name = line.split("{", 1)[0].split(" ", 1)[0]
        if name not in names:
            continue
        match = re.fullmatch(r'(sync_block_capture_[a-z_]+)(?:\{([^}]*)\})? ([^ ]+)', line)
        if match is None:
            raise IncompleteCapture("unsupported capture metric sample")
        name, raw_labels, raw_count = match.groups()
        labels = {}
        for raw_label in raw_labels.split(",") if raw_labels else []:
            label = re.fullmatch(r'([a-z_]+)="([a-zA-Z0-9_-]+)"', raw_label.strip())
            if label is None:
                raise IncompleteCapture("unsupported capture metric label")
            key, value = label.groups()
            if key in labels:
                raise IncompleteCapture("duplicate capture metric label")
            labels[key] = value
        key = (name.removeprefix("sync_block_capture_"), tuple(sorted(labels.items())))
        if key in samples:
...
def reconcile_decode_totals(metrics, process, sessions, messages):
    ...
    """
    samples = read_capture_metrics(metrics, {
        "process_info", "sessions_started", "sessions_finished", "decoded_messages"
    })
    for name, labels in samples:
        expected_labels = {"process_info": ("process_trace_id",), "decoded_messages": ("kind",)}.get(name, ())
        if tuple(label for label, _ in labels) != expected_labels:
            raise IncompleteCapture("unexpected capture metric labels")

    expected = {
        ("process_info", (("process_trace_id", process),)): 1,
        ("sessions_started", ()): sessions,
        ("sessions_finished", ()): sessions,
        **{("decoded_messages", (("kind", kind),)): count for kind, count in messages.items()},
    }
    # Exporters may include registered zero counters for unused message kinds.
    for kind in MESSAGE_KINDS:
        key = ("decoded_messages", (("kind", kind),))
        if samples.get(key) == 0:

...
| Mainnet release-state bundle tool | `zakura-utils/src/bin/zakura-release-state/main.rs` |
...
## 16. Mainnet release-state bundle

The embedded Mainnet frontier is a release artifact coupled to the terminal Mainnet checkpoint.
`zakura-release-state` creates, verifies, and imports the coupled release input. It replaces the
old RPC/raw-block checkpoint-maintenance design, which cannot operate over pruned history and
cannot produce commitment tree frontiers.

`export` opens an existing Mainnet finalized database read-only. It reads retained canonical
hashes and `BlockInfo` sizes after checkpoint 3,358,006, then calls
`produce_final_frontiers_bytes` at the captured finalized tip. This works for pruned databases
because neither input requires historical transaction bodies. Frontier production reads the
Sapling, Orchard, Sprout, and Ironwood trees, rejects a requested height other than the finalized
tip, and checks that the tip identity did not change while it was reading.

The exporter writes a staged directory containing `manifest.json`, `block-metadata.bin`, and
`mainnet-frontier.bin`, verifies that staged bundle through the same parser used by import, then
renames the directory into place. The strict manifest binds Mainnet, the fixed base height/hash
and checkpoint-prefix digest, the finalized height/hash, an RFC 3339 timestamp, and each
artifact's exact filename, size, and SHA-256 digest. Block metadata uses a bounded fixed-width
format with a header and one canonical hash/size record for every height after the base. Version
1 starts with the eight bytes `ZKRSB001`, followed by the base height, finalized height, and
record count as little-endian `u32` values. Each 36-byte record is the block hash in consensus
serialization order followed by its `u32` little-endian serialized size. Heights are implicit,
contiguous, and bounded by the header.

The base remains fixed rather than moving with the embedded frontier. Consequently a long-lived
publisher binary can produce the next bundle after earlier frontier updates merge. Import
authenticates the source checkpoint prefix through that fixed base and checks every already
committed later checkpoint against the bundle metadata. It preserves that prefix byte-for-byte,
then selects only the new suffix. The byte-count/400-block selection rule is retained, the final
bundle height is forced to be a checkpoint, and a short tail is rebalanced so every gap remains
between 17 and 400 blocks.

Import validates the complete bundle before staging replacements for:

- `zakura-chain/src/parameters/checkpoint/main-checkpoints.txt`;
- `zakura-state/src/service/finalized_state/vct/mainnet-frontier.bin`; and
- `zakura-state/src/service/finalized_state/vct/mainnet-frontier.json`.

The tracked JSON provenance binds the resulting checkpoint and frontier digests to the immutable
bundle manifest. Identical imports are no-ops. A lower height, a same-height conflict, a changed
canonical prefix, an unknown manifest field, corrupt metadata, a wrong frontier height, or any
size/digest mismatch fails closed. `verify-source --require-bundle-provenance` is the release
gate. It rejects the honest `legacy-bootstrap` provenance used for the frontier that predates
this pipeline.

The GCP checkpoint-generation and `checkpoint-update.yml` pipelines remain retired. Their manual
Mainnet frontier-output contract is superseded by two explicit workflows:

- `publish-release-state-image.yml` builds an immutable amd64 publisher image from reviewed
  `main`, containing both `zakurad` and `zakura-release-state`, and records its digest for the
  publisher deployment;
- `prepare-mainnet-release-state.yml` resolves one recent immutable bundle from the configured R2
  pointer, imports and verifies it, limits the generated diff to the three coupled source files,
  and opens or updates a draft pull request.

Release creation does not consult the moving R2 pointer. It verifies the checkpoint, frontier,
and tracked bundle provenance already reviewed and committed in the release source.

The final-frontier compatibility test remains part of this contract. It produces frontier bytes
from a small generated `FinalizedState`, loads them through the parser used by
`VCT_REGTEST_FRONTIER` and the embedded Mainnet frontier, checks all four roots and the height,
and rejects a different expected height. Bytes accepted by this path have the same format the
node parses at startup.

The focused local checks are:

```text
cargo test -p zakura-state final_frontier
cargo test -p zakura-utils --features zakura-release-state --bin zakura-release-state
cargo run -p zakura-utils --features zakura-release-state --bin zakura-release-state -- \
  verify-source --source-dir .
```
...
### 17.4 Availability and guarded replay

Current `main` deliberately sets `MAINNET_ARTIFACT` to `None`: no canonical Mainnet repair bytes
have been approved and embedded. The format, generator, validation, and upgrade machinery remain,
but current binaries cannot repair databases created by the old 28.0.0 VCT path. Startup rejects
an affected database with `VctSproutHistoryRepairRequired`; operators must discard it and perform
a fresh sync with current code. This includes pre-28.0.1 public pruned snapshots. A fresh
database maintains Sprout history correctly and does not need the repair artifact.

If a separately reviewed artifact is embedded in the future, it is loaded only through
...
databases never load or replay a Mainnet artifact. Normally synced databases and databases

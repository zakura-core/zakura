...
//!
//! zakura-consensus accepts an ordered list of checkpoints, starting with the
    ...
    I: IntoIterator<Item = String>,
{
    ...
    // including the `zakura-checkpoints` passthrough arguments.
...
/// Process entry point for `zakura-checkpoints`
#[tokio::main]
#[allow(clippy::print_stdout, clippy::print_stderr, clippy::unwrap_in_result)]
async fn main() -> Result<()> {
    eprintln!("zakura-checkpoints launched");
    ...
    for request_height in starting_height.0..height_limit.0 {
        ...
        let (hash, response_height, size) = match args.backend {
            ...
            Backend::Zakurad => {
                // get block data from zakurad (or zcashd) by deserializing the raw block

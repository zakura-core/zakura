...
fn new_ephemeral_zakura_db(network: &Network) -> ZakuraDb {
    ZakuraDb::new(
        &Config::ephemeral(),
        STATE_DATABASE_KIND,
...
fn intra_block_self_spend_chain_in_finalized_state() {
    ...
    // Synthetic block. The header is a dummy from zakura-test (round-tripped); the writer
    ...
    *existing_abl.received_mut() = u64::from(value);
    ...
    let (balance, received) = zakura_db

...
**LC-AUX-06 [LS] — Semantic auxiliary capacity.** The engine MUST retain no more than 32 distinct auxiliary payloads for one header. The semantic payload identity MUST include the header hash and every tree-aux field. It MUST exclude the delivery ID, supplier, session, request, work owner, and body-size hint. A duplicate semantic payload MUST NOT consume another slot or replace a distinct retained payload. One supplier MUST NOT consume more than one rooted semantic-payload slot for one header. Independent plan verification MUST enforce the same rules. At the per-header limit, admission MAY replace a rejected or disputed input with a distinct input. A selected repair MAY also replace an unchecked input, including a recovered row whose outcome claim was discarded. Admission MUST NOT remove an authenticated input from a retained header. Replacement MUST update the delivery row and its header index atomically. Eviction bounds input history; it grants no header or root authority.

**LC-AUX-07 [LS] — Authoritative repair progress.** A repair request MUST claim an exact contiguous selected range and durable auxiliary-requirement episode. State MUST use one target when the blocking height has durable auxiliary evidence, a resource alarm, or no adjacent empty gap. Otherwise, state MUST bound the range by the selected tip, the checkpoint handoff, the transition limit, aggregate auxiliary capacity, and the first durable auxiliary row. A range episode MUST bind every selected frontier, every authentication boundary, the state version, and the absence of durable auxiliary rows. Header sync MUST select eligible suppliers in rotating queue order, without giving larger advertised batches priority over waiting suppliers. New or reconnected suppliers MUST join the back of that queue. Header sync MAY shorten the context to the largest prefix that the selected V1 supplier can serve. State MUST validate and admit the complete prefix atomically. State MUST reject a stale claim, a partial range, a changed selected path, new auxiliary evidence, a semantic payload that state already retains, or an input excluded by committed rejection or dispute evidence without changing state. Header sync MUST rotate away from a supplier after a successful repair or a supplier-attributed failure. A correlated `Busy` response MUST pause only that supplier until a later valid `Status` arrives from the same session, while other eligible suppliers remain available. An unchanged status refresh MUST make that supplier eligible again without requiring elapsed time or a new tip. A status received before the refusal, from another supplier, or from a departed session MUST NOT release the wait. Each subsequent `Busy` MUST require another status event. Header sync MUST NOT arm a retry deadline for `Busy`. Other supplier-attributed failures MUST NOT reuse that supplier in the same session. Header sync MUST NOT use a supplier that already occupies the target's rooted slot during the same episode. Live peer admission limits MUST bound supplier history. Header sync MUST remove failure-only identity state when a supplier disconnects or its session is replaced, even if the old disconnect event has not arrived. A late local completion from the departed session MUST NOT recreate failure history for its replacement. A transient local failure MAY retry after bounded backoff. Header sync MUST wait when state reports that repair admission lacks capacity. It MAY resume only after a newer state reports available capacity. When no eligible supplier remains, header sync MUST wait for a busy supplier’s later status, a new supplier, target, or episode. Periodic status publication under LC-WIRE-17 provides retry opportunities when the supplier’s tip is unchanged. Auxiliary repair failure MUST NOT affect fork choice.

**LC-AUX-08 [LS] — Commit capacity reserve.** Every integrated transition MUST preserve aggregate auxiliary capacity for the finalized header and its first two selected successors. The reserve MUST cover each header's unfilled per-header slots, including successors that have not arrived. A fork change without new auxiliary data MUST enforce the reserve for its new selected window. Retention MAY evict an unprotected header that holds auxiliary input, together with its descendants, to restore the reserve. Auxiliary pressure MUST NOT evict any other header. Retention MUST preserve selected, verified, and authenticated context paths. Admission MUST refuse a transition when protected evidence prevents reserve recovery. A store that already lacks the reserve MAY commit a transition that keeps or reduces its deficit, so startup and finality can drain protected input that predates the reserve. Such a transition MUST NOT increase the deficit. Deliveries inside the window MUST remain subject to the hard aggregate and per-header limits. State repair preflight MUST account for the reserve and permitted input replacement. Under capacity pressure, state MUST request the single blocking target before a speculative repair suffix. The authentication sweep MUST withdraw requests beyond the commit window when speculative capacity is exhausted. It MUST preserve the committer's repair request.

A capacity blockage MUST start its absolute thirty-minute deadline when the task enters the blocked state. Peer events and unsuccessful context rechecks MUST NOT postpone that deadline. A positive capacity context MUST clear the deadline before immediate assignment can hide the resolved state. A later blockage MUST start a new deadline. Expiry MUST produce one fatal event for daemon shutdown. The deadline MUST observe the latest writer repair signal before shutdown. A new committed version alone MUST NOT count as recovered capacity. Restart MUST NOT imply that durable saturation has been repaired.
...
**LC-WIRE-06 [ZW] — No target substitution.** The accepted target MUST remain snapshot-bound even if the server’s selected tip changes concurrently. The server MUST either complete that exact path through one or more responses/continuations or return one explicit `HeadersOutcome`; it MUST NOT silently substitute its new selected path. A local generation or finality advance alone MUST NOT cancel acquisition or serving of an unchanged exact path. Serving leases MUST NOT prevent local retention from evicting peer-requested forks to admit better-work branches. Retained ancestry lookup MUST run outside the state writer and transition-engine locks against a coherent snapshot, stop when the first matching locator is established, and recheck target availability when installing the serving cursor. Pruning MAY make an uncaptured page unavailable, but MUST NOT invalidate a page snapshot already captured. Page reads MUST use a coherent bounded snapshot without requiring an unchanged global state version. State MUST consume the serving lease before returning each successful page, including incomplete pages. A delayed release of that lease MUST NOT revoke a later acquisition. A continuation MUST reacquire the requested target by hash and MUST NOT reserve capacity between requests. State MAY reuse a completed page's immutable path index only for the same session, target, and next locator, after rechecking target availability and acquiring fresh ownership. This cache MUST hold at most 16 indexes, each bounded by the retained node limit, and MUST NOT hold disk snapshots or retention roots. Cache pressure MUST evict an idle index rather than refuse serving capacity. Finalized fallback bounds and locator selection still apply. Session, lease, request, cursor, expiry, and capacity checks still apply. Finalized auxiliary records looked up by height MUST match the served path by hash before attachment.
...
- **PW-04 `outcomes_and_attribution`:** all four explicit outcomes, stale refresh, busy status wait, malformed ancestor, mismatched target/ID, invalid header, and score/no-score assertions.

...
pub async fn run() -> Result<()> {
    ...
    let (mut zakurad, zakura_rpc_address) = if let Some(zakurad_and_address) =
        ...
        tracing::info!(
            ?network,
            ?test_type,
            "running gRPC query tests using lightwalletd & zakurad...",
        );
    ...
    tracing::info!(
        ?test_type,
        ?zakura_rpc_address,
        "launched zakurad, waiting for zakurad to open its RPC port..."
    );
    ...
    if !DATABASE_FORMAT_UPGRADE_IS_LONG {
        wait_for_state_version_upgrade(
            ...
            [format!("Opened RPC endpoint at {zakura_rpc_address}")],
        )?;
    }
    ...
    let (lightwalletd, lightwalletd_rpc_port) =
        spawn_lightwalletd_for_rpc(network.clone(), test_name, test_type, zakura_rpc_address)?
    ...
    let (_lightwalletd, mut zakurad) = wait_for_zakurad_and_lightwalletd_sync(
        lightwalletd,
        lightwalletd_rpc_port,
        zakurad,
        zakura_rpc_address,
    ...
    let hash = zakura_test::vectors::BLOCK_MAINNET_1_BYTES
    ...
    assert_eq!(
        ...
        *zakura_test::vectors::SAPLING_TREESTATE_MAINNET_419201_STRING
    );
    ...
    assert_eq!(
        ...
        zakurad::application::user_agent()
    );

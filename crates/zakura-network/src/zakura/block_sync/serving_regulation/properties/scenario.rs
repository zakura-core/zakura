...
// One maximum-size Block, its message tag, and one height/count ending.
const MAXIMUM_BLOCK_BYTES: u64 = 2_000_000;
const MESSAGE_TAG_BYTES: u64 = 1;
pub(super) const TERMINAL_PAYLOAD_BYTES: u64 = MESSAGE_TAG_BYTES + 4 + 4;
pub(super) const RESPONSE_CAP: u64 =
    MAXIMUM_BLOCK_BYTES + MESSAGE_TAG_BYTES + TERMINAL_PAYLOAD_BYTES;
...
impl Limit {
    ...
    pub(super) fn config(self) -> ZakuraBlockSyncConfig {
        ...
        policy.node_active_requests = self.node_capacity();
        config
    }
}

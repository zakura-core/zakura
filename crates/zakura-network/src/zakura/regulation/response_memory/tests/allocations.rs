...
fn response_authorization_funds_allocation_before_retaining_it() {
    ...
    let scope_setup = WriterFence::setup_bytes_for_test();
    ...
    let (scope, allocated) =
        measure(|| WriterFence::try_with_memory(&cancel, &cause, memory.clone()).unwrap());
    assert_eq!(node.reserved_for_test(), setup);
    assert!(allocated.retained_bytes > 0);
    assert!(u64::try_from(allocated.peak_live_bytes).unwrap() <= scope_setup);
    let (authorization, allocated) = measure(|| scope.try_open().unwrap());
    let funded = node.reserved_for_test() - setup;
    assert!(allocated.retained_bytes > 0);
    assert!(u64::try_from(allocated.retained_bytes).unwrap() <= funded);
    ...
    let (denied, allocated) = measure(|| scope.try_open());
    assert!(matches!(denied, Err(ExchangeOpenError::MemoryFull)));
    ...
    let (denied, allocated) =
        measure(|| WriterFence::try_with_memory(&cold_cancel, &cold_cause, memory.clone()));
    assert!(matches!(denied, Err(ExchangeOpenError::MemoryFull)));
    assert_eq!(allocated.requested_bytes, 0);
...
fn cold_response_setup_and_unfinished_cleanup_fit_the_fixed_allowances() {
    ...
    let (funded, allocated) = measure(|| {
        ...
        let scope = WriterFence::try_with_memory(&cancel, &cause, memory).unwrap();
...
fn response_authorization_transitions_do_not_allocate_unfunded_storage() {
    ...
    let setup = ResponseMemory::node_setup_bytes_for_test()
        ...
        + WriterFence::setup_bytes_for_test();
    ...
    let mut authorization = scope.try_open().unwrap();
    let writer = authorization.writer();

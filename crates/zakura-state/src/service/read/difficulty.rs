...
fn adjust_difficulty_and_time_for_testnet(
    ...
    // `previous_block_time + 6 * PoWTargetSpacing`. Switching earlier would
    // future-date the block solely to obtain minimum difficulty, producing
    // spurious minimum-difficulty blocks on Testnet.
    //
...
mod tests {
    ...
    fn recent_block_data(network: &Network) -> Vec<(CompactDifficulty, DateTime<Utc>)> {
        ...
        (0..POW_ADJUSTMENT_BLOCK_SPAN)
            .map(|offset| {
                (
                    threshold,
                    DateTime32::from(PREVIOUS_BLOCK_TIME - offset as u32).into(),
                )
            })
            .collect()
    }
    ...
    #[test]
    fn time_past_threshold_selects_minimum_difficulty() {
        let network = Network::new_default_testnet();

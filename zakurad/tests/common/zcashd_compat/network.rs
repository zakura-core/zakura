...
/// Verifies that zakurad is connected to at least one peer on the network.
///
/// Calls `getpeerinfo` on zakurad and asserts a non-empty peer list.
///
...
pub async fn peer_connectivity() -> Result<()> {
    ...
    assert!(
        !peers.is_empty(),
        "zakurad has no connected peers on {network}; is it actually synced?",
...
/// Verifies that zakurad and zcashd agree on the hash of the block at height 1.
///

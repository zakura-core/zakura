...
- Zakura's continuous integration checks that tier 2 platforms will always build,
  but they may or may not pass tests.
- Zakura's continuous integration checks that tier 1 platforms will always build
  and pass tests.

Adding a new tier 3 platform imposes minimal requirements; but we focus
primarily on avoiding disruption to ongoing Zakura development.
...
minimize any disruptions to ongoing Zakura development.
...
determined by the judgment of the Zakura team.
...
precise interpretation of "reasonable amount of time" up to the Zakura team.

The availability or tier of a platform in stable Zakura is not a hard stability
...
Zakura team may grant an exception for good reason. The word "may" indicates
...
At this tier, the Zakura project provides no official support for a platform, so
...
- Patches adding or updating tier 3 platforms must not break any existing tier 2
  ...
  without approval of either the Zakura team of the other tier 3 platform.
...
of the Zakura codebase, we may post a PR to remove support for that platform. Any such PR will be CCed
...
At this tier, the Zakura project guarantees that a platform builds, and will reject
patches that fail to build on a platform. Thus, we place requirements that ensure
the platform will not block forward progress of the Zakura project.

A proposed new tier 2 platform must be reviewed and approved by Zakura team based
...
- Tier 2 platforms must implement all the Zcash consensus rules.
  Other Zakura features and binaries may be disabled, on a case-by-case basis.
...
- The platform must not place undue burden on Zakura developers not specifically
  concerned with that platform. Zakura developers are expected to not gratuitously
...
- The platform must provide documentation for the Zcash community explaining how
  ...
  If at all possible, this documentation should show how to run Zakura programs
...
- The platform must build reliably in CI, for all components that Zakura's CI
  considers mandatory.
  - Since a working Rust compiler is required to build Zakura,
    the platform must be a [Rust tier 1 platform](https://rust-lang.github.io/rustup-components-history/).
- The Zakura team may additionally require that a subset of tests pass in
...
- Test failures on tier 2 platforms will be handled on a case-by-case basis.
  Depending on the severity of the failure, the Zakura team may decide to:
...
At this tier, the Zakura project guarantees that a platform builds and passes all
...
A proposed new tier 1 platform must be reviewed and approved by the Zakura team
...
- Tier 1 platforms must implement Zakura's standard production feature set,
  including the network protocol, mempool, cached state, and RPCs.
  Exceptions may be made on a case-by-case basis.
  - Zakura must have reasonable security, performance, and robustness on that platform.
    These requirements are subjective, and determined by consensus of the Zakura team.
...
- The platform must serve the ongoing needs of multiple production
  users of Zakura across multiple organizations or projects. These requirements
  are subjective, and determined by consensus of the Zakura team. A tier 1
...
- The platform must build and pass tests reliably in CI, for all components that
  Zakura's CI considers mandatory.
...
- If running the testsuite requires additional infrastructure (such as physical
  ...
  such resources to the Zakura project, to the satisfaction and approval of the
  Zakura devops team.
  ...
  - If the platform requires the use of emulation to meet any of the tier
    requirements, the Zakura team must have high
    ...
    implementation of the platform, or the Zakura implementation for the platform.
  - If it is not possible to run the platform via emulation, these resources
    must additionally be sufficient for the Zakura devops team to make them
    available for access by Zakura team members, for the purposes of development
    ...
    Zakura developers to test the platform, but does not obligate other Zakura
    developers to make platform-specific fixes.)
  - Resources provided for CI and similar infrastructure must be available for
    continuous exclusive use by the Zakura project. Resources provided
    for access by Zakura team members for development and testing must be
...
features or OS version required) requires the approval of the Zakura team, and

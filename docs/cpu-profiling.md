...
The `historical_pruned_checkpoint` workload creates a fresh copy of the baked archive state for every invocation. Before measurement it runs the existing offline pruning command with the 10,000-block minimum retention, then syncs 1,000 additional blocks in pruned checkpoint mode. That short preparation sync drains the retained archive backlog and initializes the VCT path. Pruning is capped at 20 minutes and preparation sync at 10 minutes; a failure or missing confirmation of the exact preparation height stops the benchmark. Neither interval is included in throughput.

The measured start is therefore 1,708,210 with the standard baked state, and the requested stop must exceed it. Preparation logs and the separate preparation config are included in each artifact. The measured node restarts from that state with its normal metrics counters reset and the page cache cleared. This is a historical pruned checkpoint workload; it does not reproduce a database's entire file history from a pruned genesis-to-tip sync. Each candidate uses its own binary during preparation, so compare the resulting logical tips and retain that preparation detail when interpreting results.

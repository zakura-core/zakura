...
Every release tag must match the `zakura` package version: release binaries are built without `.git`, so `zakurad --version` reports the package version, not the tag. Create tags using the protected `create-release.yml` workflow, which verifies the package version before creating the tag and GitHub release. The `release-binaries.yml` workflow independently checks the version again before building or publishing assets. See [Protected Release Tags](../../../docs/release-tag-protection.md) for configuration and operation details.

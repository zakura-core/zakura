...
| Published Mainnet frontier grid, and its packaging | the `zakura-assets` workspace dependency in the root `Cargo.toml`; `assets/zakura-assets/`, `scripts/pack-assets-crate.py` |
...
crates.io package built from `assets/zakura-assets/`, so ~4.7 MB of regenerated bytes stay out of

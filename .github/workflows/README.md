...
- **`zakura-pr-node.yml`** — ephemeral one-hour real-node test of a PR or ref: boots a droplet from the pre-baked image, attaches a chain-state snapshot clone (`tip`, `pre-checkpoint`, `sandblast`, or `genesis`), builds the branch incrementally, runs it, and posts a metrics summary as a PR comment. `pre-checkpoint` picks the newest retained tip snapshot whose baked state height (the `-h<height>` suffix the bake stamps into snapshot names) is below the branch's max checkpoint, so the node syncs through the last checkpoint handoff (the VCT frontier write) with the shortest possible run — the standard check for a branch that advances the checkpoints, such as a release-state bundle PR. It fails before creating a droplet when no retained snapshot is below the checkpoint, and fails the run if the started node cannot prove a crossing, so a green run always means a real crossing. The droplet stays up for SSH inspection until the reaper removes it.

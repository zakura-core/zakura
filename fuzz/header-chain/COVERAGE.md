...
| Header/full-state fork differential | Shared observable rules plus combined full-state/header writer before finalization | LC-PARITY-01..03, LC-SELECT-01..03 | `df_01_observable_activation_headers_pass_shared_rules`, `df_01_body_valid_fork_graph_matches_full_state_before_finalization`, `df_01_generated_nu5_graph_matches_full_state_before_finalization` | Covered: real production headers exercise every commitment-format activation through NU5 on both networks; body-valid production forks compare work, raw-hash tie order, and selected tips; and one coherent custom graph is authenticated through Canopy, then passes the same sealed header and full-state paths across NU5 without another finalization |

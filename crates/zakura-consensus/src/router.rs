...
#[instrument(skip(network, state_service, transaction_state, mempool))]
    ...
    let state_checkpoint_verify_handle = tokio::task::spawn(
        ...
        async move {
            ...
            for (height, checkpoint_hash) in full_checkpoints.iter() {
                ...
                let request = zakura_state::Request::BestChainBlockHash(*height);
                ...
                match checkpoint_state_service.oneshot(request).await {
                    ...
                    Ok(zakura_state::Response::BlockHash(None)) => {
                        if checkpoint_sync {
                            tracing::info!(
...
/// The background task handles for `zakura-consensus` verifier initialization.
#[derive(Debug)]

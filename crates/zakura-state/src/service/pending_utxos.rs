...
pub struct PendingUtxos(
    Arc<Mutex<HashMap<transparent::OutPoint, broadcast::Sender<transparent::Utxo>>>>,
);
...
impl PendingUtxos {
    ...
    pub fn queue(
        ...
        let mut receiver = self
            .0
            .lock()
            .expect("pending UTXO lock is not poisoned")
            .entry(outpoint)
            .or_insert_with(|| {
    ...
    pub fn prune(&mut self) {
        self.0
            .lock()
            .expect("pending UTXO lock is not poisoned")
            .retain(|_, chan| chan.receiver_count() > 0);
    }

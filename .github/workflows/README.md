...
| `test-docker.yml` | Builds the production runtime image once, then smoke-tests its packaged binaries, privilege drop, default startup, and combined config overrides. | `A-release` PRs on Rust/Cargo/Docker paths, pushes on Cargo/Docker paths, weekly, manual |

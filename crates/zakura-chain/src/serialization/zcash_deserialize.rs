...
pub(super) fn read_external_count<R: io::Read, T: ZcashDeserialize + TrustedPreallocate>(
    ...
    // Keep the existing initial allocation cap even when the count fits.
    // A plausible count does not prove the elements are valid, and decoded
    // elements can be much larger than their minimum encoding.
...
pub(super) fn read_bytes<R: io::Read>(
    ...
    // Unknown streams retain the existing incremental growth behavior.
    // Never reserve the entire peer-declared count before receiving its bytes.
    let mut vec = Vec::with_capacity(external_count.min(MAX_INITIAL_ALLOCATION));

...
use crate::application::{build_version, ZakuradApp};
...
impl Metadata {
        ...
        F: Fn(&str) -> Option<String>,
    {
        ...
        let git_sha = lookup_value(&lookup, "GITHUB_SHA")
            .or_else(|| ZakuradApp::git_commit().map(ToOwned::to_owned));
...
fn release_name() -> String {
    release_name_from(&env_var, ZakuradApp::git_commit())
}
...
mod tests {
    ...
    fn release_name_prefers_sentry_release_override() {
        let env = HashMap::from([("SENTRY_RELEASE", "zakurad@4.4.0-rc.1".to_string())]);

        let release = release_name_from(&|key| env.get(key).cloned(), Some("deadbeef"));

        assert_eq!(release, "zakurad@4.4.0-rc.1");
    }
    ...
    fn client_options_enables_logs_and_roundtrips_environment() {
        ...
        let options = metadata.client_options_with_release("zakurad@test".to_string());

        assert!(options.enable_logs);
        assert_eq!(options.environment.as_deref(), Some("stage"));
        assert_eq!(options.release.as_deref(), Some("zakurad@test"));
    }

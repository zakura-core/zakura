//! Check one complete response using the real discovery codec. GetPeers has no
//! request ID on the wire, so this test adapter permits one outstanding query
//! per receiver. Production discovery has not yet adopted these response rules.

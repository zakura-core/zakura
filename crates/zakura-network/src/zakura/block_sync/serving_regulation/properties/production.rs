...
pub(super) struct Production {
    ...
    /// Mutation control: simulate a writer that releases its guard before I/O ends.
    pub(super) drop_write_guard: bool,
}
    ...
    pub(super) async fn apply(&mut self, action: &Action) -> Outcome {
        let outcome = match *action {
            ...
            Action::BeginWrite { session } => {
                ...
                let write = |_frame| async move {
                    match completion.await {
                        Ok(true) => Ok(()),
                        _ => Err(()),
                    }
                };
                let mut task: Pin<Box<dyn Future<Output = Result<(), ()>>>> =
                    if self.drop_write_guard {
                        let (frame, guard) = frame.into_parts();
                        drop(guard);
                        Box::pin(write(frame))
                    } else {
                        Box::pin(frame.write_with(write))
                    };
                assert!(

...
**LC-AUX-08 [LS] — Commit capacity reserve.** Integrated admission MUST reserve aggregate auxiliary capacity for the finalized header and its first two selected successors. The reserve MUST cover each header's unfilled per-header slots, including successors that have not arrived. Admission MUST reject new deliveries outside this window when their post-retention count would consume the reserve. Deliveries inside the window MUST remain subject to the existing hard aggregate and per-header limits. Retention MUST preserve protected evidence. Under capacity pressure, state MUST request the single blocking target before a speculative repair suffix. A continuous thirty-minute wait for local capacity MUST produce one fatal event for daemon shutdown. The deadline MUST observe the latest writer repair signal before shutdown. A newer committed state at the deadline MUST receive a context recheck before shutdown. Context rechecks MUST preserve the capacity deadline until state reports available capacity or retires the repair generation. Restart MUST NOT imply that durable saturation has been repaired.

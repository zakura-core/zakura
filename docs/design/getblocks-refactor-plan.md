...
The [audit stack](getblocks-stack.md) implements this plan. Its final draft,
PR #945, selects the paired version normally and removes the old serving driver.
It must remain unmerged until the separate Iroh upgrade in #935 is integrated
and the required transport gate passes on that combined code. Earlier
[measurements](getblocks-refactor-results.md) used a different transport revision;
they do not establish readiness for the upgraded dependency.
...
Start from a refreshed #892 head. The transport prerequisite is now the separate Iroh upgrade in #935; the former backport in #941 was withdrawn. Record baseline test results, memory use, and useful block throughput for comparison.
...
Read incoming setup without blocking connection cancellation, pair expiry, or
worker cleanup. Reserve service capacity only after the first role's pair identity
arrives. After an incomplete pair expires, defer that connection's next offer for
one setup interval so waiting sessions can claim the released capacity. A complete
...
temporary raw-stream probes; retain the version-4
ownership histories.
...
The `blocksync-regression` profile covers the fixed regressions in this refactor. Generated ownership histories and version-4 JSON replay belong to the follow-on property-testing PR, #896. Confirm the intended tests actually run. Validate integration with the separate Iroh upgrade in #935; do not restore the withdrawn #941 backport or its crate-family pins. Run the relevant documentation/configuration checks too.

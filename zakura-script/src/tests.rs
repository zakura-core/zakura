...
fn verify_valid_script(nu: NetworkUpgrade, tx: &[u8], amount: u64, pubkey: &[u8]) -> Result<()> {
    let transaction = tx.zcash_deserialize_into::<Arc<zakura_chain::transaction::Transaction>>()?;
...
fn fail_invalid_script() -> Result<()> {
    ...
    let transaction =
        SCRIPT_TX.zcash_deserialize_into::<Arc<zakura_chain::transaction::Transaction>>()?;
...
fn count_coinbase_legacy_sigops_includes_coinbase_script() -> Result<()> {
    ...
    let output_amount = zakura_chain::amount::Amount::try_from(1_000_000)?;
...
/// `bad-blk-sigops`.
///
/// This test exercises the free function `zakura_script::p2sh_sigop_count` directly on a synthetic
...
fn p2sh_sigop_count_counts_redeem_script() -> Result<()> {
    ...
    let input = transparent::Input::PrevOut {
        outpoint: transparent::OutPoint {
            hash: zakura_chain::transaction::Hash([0u8; 32]),
            index: 0,
        },
        unlock_script,
        sequence: u32::MAX,
    ...
    let spent_output = transparent::Output {
        value: zakura_chain::amount::Amount::try_from(1_000_000)?,
        lock_script,
    };
...
/// `zakura_consensus::block::verify_block` (`block.rs` `sigops += response.sigops()`), where each
/// transaction's contribution is `tx.sigops() + cached.p2sh_sigops()` (set in
/// `zakura_consensus::transaction.rs`'s `Block`-path response).
///
/// Asserts:
...
fn block_sigop_total_includes_coinbase_and_p2sh() -> Result<()> {
    ...
    /// Matches `zakura_consensus::MAX_BLOCK_SIGOPS`. Hard-coded to avoid a reverse dependency on
    /// `zakura-consensus`.
    ...
    // `zakura_consensus::block::verify_block` via `response.sigops()`, where the response is built
    // in `zakura_consensus::transaction.rs`'s `Block`-path arm as `tx.sigops() +
    // cached.p2sh_sigops()`.
    //
...
fn is_valid_rejects_mismatched_previous_outputs_length() {
    ...
    let transaction = SCRIPT_TX
        .zcash_deserialize_into::<Arc<zakura_chain::transaction::Transaction>>()
        .expect("test fixture deserializes");
...
fn poc_p2sh_1001_accurate_multisigs_should_stay_below_block_sigop_limit() -> Result<()> {
    ...
    let inputs: Vec<_> = (0..SPENDS)
        .map(|i| {
            ...
            transparent::Input::PrevOut {
                outpoint: transparent::OutPoint {
                    hash: zakura_chain::transaction::Hash(hash),
                    index: 0,
                },

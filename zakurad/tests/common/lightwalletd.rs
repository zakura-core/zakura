//! `lightwalletd`-specific shared code for the `zakurad` acceptance tests.
//!
//! # Warning
//!
...
use super::{config::testdir, launch::ZakuradTestDirExt, test_type::TestType};
...
pub fn zebra_skip_lightwalletd_tests() -> bool {
    ...
    //
    // See is_command_available() in zakura-test/src/tests/command.rs for one way to do this.
...
/// Spawns a lightwalletd instance on `network`, connected to `zakurad_rpc_address`,
...
pub fn spawn_lightwalletd_for_rpc<S: AsRef<str> + std::fmt::Debug>(
    ...
    let lightwalletd_dir = testdir()?.with_lightwalletd_config(zakurad_rpc_address)?;
...
pub trait LightWalletdTestDirExt: ZakuradTestDirExt
where
    Self: AsRef<Path> + Sized,
{
    ...
    fn with_lightwalletd_config(self, zakura_rpc_listener: SocketAddr) -> Result<Self>;
}
    ...
    Self: TestDirExt + AsRef<Path> + Sized,
{
    ...
    fn with_lightwalletd_config(self, zakura_rpc_listener: SocketAddr) -> Result<Self> {
        ...
        let lightwalletd_config = format!(
            ...
            rpcpassword=xxxxx
            ",
            zakura_rpc_listener.ip(),
            zakura_rpc_listener.port(),
        );

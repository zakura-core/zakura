...
fn late_body_claim_has_exactly_one_winner() {
    ...
    assert_eq!(
        outcomes
            .iter()
            .filter(|outcome| { **outcome == LateBodyClaim::ClaimedPending { reset_epoch: 0 } })
            .count(),
        1,
    );
    assert_eq!(
        outcomes
            .iter()
            .filter(|outcome| **outcome == LateBodyClaim::AlreadyReceived)
            .count(),
        1,
    );
...
fn destructive_reset_and_claim_are_atomic_in_both_lock_orderings() {
    ...
    // and removes that old ownership before either operation returns.
    {
        ...
        assert_eq!(claim, LateBodyClaim::ClaimedPending { reset_epoch: 0 });
        assert_eq!(reset.1, 1);
    ...
    // the epoch, the replacement is inserted and claimed under epoch 1.
    {
        ...
        assert_eq!(claim, LateBodyClaim::ClaimedPending { reset_epoch: 1 });
        assert!(queue.in_flight_contains(height));
...
fn work_queue_reserved_bytes_counter_matches_scan_across_transitions() {
    ...
    queue.release_reserved_heights([block::Height(3)], owner);
...
fn watchdog_between_registry_check_and_settlement_preserves_late_body() {
    ...
    // The old active-only release reports no reservation. The authoritative
    // late claim must instead reacquire the pending height without resurrecting
    // a request-budget charge.
    assert_eq!(queue.release_active_reserved_height(height), None);
    assert_eq!(
        queue.claim_late_body(height, hash, block::Hash([0; 32]), true),
        LateBodyClaim::ClaimedPending { reset_epoch: 0 },
    );
    assert_eq!(budget.reserved(), 0);
    assert!(!queue.pending_contains(height));
...
fn claim_received_does_not_orphan_a_charge() {
    ...
    let released = queue.release_reserved_heights([block::Height(1)], owner);
...
fn release_reserved_heights_skips_received_body_owned_by_sequencer() {
    ...
    // height 1 is skipped and stays in `in_flight`.

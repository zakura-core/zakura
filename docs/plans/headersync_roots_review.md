...
- **zakurad lib tests did not compile.** `start.rs`'s `zakura_header_sync_driver_tests`
  imports `block_roots_cover_range` and `root_covered_query_best_header_tip` via
  `super::zakura::`, but `zakurad/src/commands/start/zakura/mod.rs` never re-exported them
  ...
  reaching zakurad, so the zakurad tests never compiled. Fix: added both to the
...
1. **`zakura-state` proptest `service::finalized_state::tests::prop::vct_frozen_frontier_survives_reopen`.**
...
2. **`zakurad` legacy block-sync vectors (run via nextest):**
...
3. **`zakura-network` testkit network tests (env-flaky):**
...
- `cargo test -p zakura --lib` (single process) cascades ~76 failures from one root panic:
  `zakura_test::init()` → color-eyre `install().unwrap()` → "a hook has already been
  installed", poisoning the init `Once`. CI uses **nextest** (process-per-test), which
  sidesteps this. Always validate zakurad with `cargo nextest run`, not `cargo test --lib`.
- `cargo clippy --workspace -- -D warnings` fails on **pre-existing** zakura-chain lints

...
const SUPPORTED_SPEC_VERSION: &str = "1.5";
const EXPECTED_RULE_COUNT: usize = 118;
...
const REPOSITORY_SCAN_LIMITS: ScanLimits = ScanLimits {
    max_depth: 32,
    max_entries: 100_000,
    max_rust_files: 4_096,
...
pub fn run(repo_root: &Path, requested_rule: Option<&str>) -> Result<(), Box<dyn Error>> {
    let spec = read_repository_document(repo_root, SPEC_PATH)?;
    let manifest = read_repository_document(repo_root, MANIFEST_PATH)?;
...
fn read_repository_document(repo_root: &Path, relative: &str) -> Result<String, ValidationError> {
    ...
    {
        return Err(ValidationError(format!(
            "conformance document path `{relative}` is not repository-relative"
        )));
    }
    ...
    for (index, component) in components.iter().enumerate() {
        ...
        let metadata = fs::symlink_metadata(&path).map_err(|error| {
            ValidationError(format!("failed to inspect `{}`: {error}", path.display()))
        })?;
        if metadata.file_type().is_symlink() {
            return Err(ValidationError(format!(
                "conformance document path `{}` contains a symbolic link",
                path.display()
            )));
        }
        let is_document = index + 1 == components.len();
        if is_document && !metadata.is_file() {
            return Err(ValidationError(format!(
                "conformance document `{}` is not a regular file",
                path.display()
            )));
        }
        if !is_document && !metadata.is_dir() {
            return Err(ValidationError(format!(
                "conformance document parent `{}` is not a directory",
                path.display()
            )));
        }
    }

    read_bounded_utf8_file(&path, MAX_DOCUMENT_BYTES)
}

fn read_bounded_utf8_file(path: &Path, max_bytes: u64) -> Result<String, ValidationError> {
    let metadata = fs::symlink_metadata(path).map_err(|error| {
        ValidationError(format!("failed to inspect `{}`: {error}", path.display()))
    })?;
    if !metadata.is_file() {
    ...
    if metadata.len() > max_bytes {
        return Err(ValidationError(format!(
            "file `{}` exceeds the {max_bytes}-byte conformance limit",
            path.display()
        )));
    }
    ...
    fs::File::open(path)
        .map_err(|error| ValidationError(format!("failed to open `{}`: {error}", path.display())))?
    ...
    if u64::try_from(bytes.len()).unwrap_or(u64::MAX) > max_bytes {
        return Err(ValidationError(format!(
            "file `{}` grew beyond the {max_bytes}-byte conformance limit",
            path.display()
        )));
    }
    String::from_utf8(bytes).map_err(|error| {
        ValidationError(format!("file `{}` is not UTF-8: {error}", path.display()))
    })
}
...
fn collect_test_functions(
    ...
    collect_test_functions_with_limits(repo_root, REPOSITORY_SCAN_LIMITS)
}

fn collect_test_functions_with_limits(
    repo_root: &Path,
    limits: ScanLimits,
) -> Result<BTreeMap<String, Vec<PathBuf>>, ValidationError> {
    let mut tests: BTreeMap<String, Vec<PathBuf>> = BTreeMap::new();
    let mut directories = vec![(repo_root.to_path_buf(), 0_usize)];
    let mut entry_count = 0_usize;
    let mut rust_file_count = 0_usize;
    let mut total_bytes = 0_u64;
    let mut symbol_count = 0_usize;

    while let Some((directory, depth)) = directories.pop() {
        let entries = fs::read_dir(&directory).map_err(|error| {
            ValidationError(format!("failed to read `{}`: {error}", directory.display()))
        })?;
        for entry in entries {
            ...
            if file_type.is_dir() {
                ...
                if entry_depth > limits.max_depth {
                    return Err(ValidationError(format!(
                        "repository scan exceeds the {}-component depth limit at `{}`",
                        limits.max_depth,
                        entry_path.display()
                    )));
                }
                directories.push((entry_path, entry_depth));
                continue;
            }
            ...
            rust_file_count = rust_file_count.saturating_add(1);
            if rust_file_count > limits.max_rust_files {
                return Err(ValidationError(format!(
                    "repository scan exceeds the {}-Rust-file limit",
                    limits.max_rust_files
                )));
            }
            let source = read_bounded_utf8_file(&entry_path, limits.max_file_bytes)?;
            total_bytes = total_bytes
                .saturating_add(u64::try_from(source.len()).expect("Rust source length fits u64"));
            if total_bytes > limits.max_total_bytes {
                return Err(ValidationError(format!(
                    "repository scan exceeds the {}-byte aggregate Rust source limit",
                    limits.max_total_bytes
                )));
            }
            let remaining_symbols = limits.max_symbols.saturating_sub(symbol_count);
            let symbols = parse_test_function_names_with_limit(&source, remaining_symbols)
                .ok_or_else(|| {
                    ValidationError(format!(
                        "repository scan exceeds the {}-test-symbol limit",
                        limits.max_symbols
                    ))
                })?;
...
fn parse_test_function_names(source: &str) -> Vec<String> {
    parse_test_function_names_with_limit(source, usize::MAX)
        .expect("an unbounded symbol scan cannot exceed its limit")
}

fn parse_test_function_names_with_limit(source: &str, max_symbols: usize) -> Option<Vec<String>> {
...
mod tests {
    ...
    fn checked_in_repo_root() -> PathBuf {
        Path::new(env!("CARGO_MANIFEST_DIR"))
            ...
            .expect("xtask should live two levels below the repository root")
            .to_path_buf()
    }

    fn permissive_scan_limits() -> ScanLimits {
        ScanLimits {
            max_depth: usize::MAX,
            max_entries: usize::MAX,
            max_rust_files: usize::MAX,
            max_file_bytes: u64::MAX - 1,
...
"#;
    ...
    #[cfg(unix)]
    #[test]
    fn conformance_documents_reject_symbolic_links() {
        use std::os::unix::fs::symlink;

        let repo = tempfile::tempdir().expect("temporary repository is created");
        let external = tempfile::NamedTempFile::new().expect("external fixture is created");
        for relative in [SPEC_PATH, MANIFEST_PATH] {
            let path = repo.path().join(relative);
            fs::create_dir_all(path.parent().expect("document path has a parent"))
                .expect("document parent is created");
            symlink(external.path(), &path).expect("document symlink is created");
            let error = read_repository_document(repo.path(), relative)
                .expect_err("conformance documents must not follow symbolic links");
            assert!(error.to_string().contains("symbolic link"));
        }
    }

    #[test]
    fn repository_evidence_scan_enforces_every_resource_limit() {
        ...
        let entries = tempfile::tempdir().expect("entry-limit repository is created");
        write(entries.path(), "one.rs", "fn one() {}\n");
        write(entries.path(), "two.rs", "fn two() {}\n");
        let error = collect_test_functions_with_limits(
            entries.path(),
            ScanLimits {
                max_entries: 1,
                ..permissive_scan_limits()
            },
        )
        .expect_err("the repository entry limit is exact");
        assert!(error.to_string().contains("entry limit"));

        let depth = tempfile::tempdir().expect("depth-limit repository is created");
        write(depth.path(), "one/two/test.rs", "fn deep() {}\n");
        let error = collect_test_functions_with_limits(
            depth.path(),
            ScanLimits {
                max_depth: 2,
        ...
        .expect_err("the repository depth limit is exact");
        assert!(error.to_string().contains("depth limit"));

        let files = tempfile::tempdir().expect("file-limit repository is created");
        write(files.path(), "one.rs", "fn one() {}\n");
        write(files.path(), "two.rs", "fn two() {}\n");
        let error = collect_test_functions_with_limits(
            files.path(),
            ScanLimits {
                max_rust_files: 1,
        ...
        .expect_err("the Rust file limit is exact");
        assert!(error.to_string().contains("Rust-file limit"));

        let individual = tempfile::tempdir().expect("file-size repository is created");
        write(individual.path(), "large.rs", "#[test]\nfn large() {}\n");
        let error = collect_test_functions_with_limits(
            individual.path(),
            ScanLimits {
                max_file_bytes: 8,
        ...
        .expect_err("the individual Rust file limit is exact");
        assert!(error.to_string().contains("8-byte conformance limit"));

        let aggregate = tempfile::tempdir().expect("aggregate-size repository is created");
        write(aggregate.path(), "one.rs", "fn one() {}\n");
        write(aggregate.path(), "two.rs", "fn two() {}\n");
        let error = collect_test_functions_with_limits(
            aggregate.path(),
            ScanLimits {
                max_total_bytes: 20,
        ...
        .expect_err("the aggregate Rust source limit is exact");
        assert!(error.to_string().contains("aggregate Rust source limit"));

        let symbols = tempfile::tempdir().expect("symbol-limit repository is created");
        write(
            symbols.path(),
            "tests.rs",
            "#[test]\nfn one() {}\n#[test]\nfn two() {}\n",
        ...
        .expect_err("the test symbol limit is exact");
        assert!(error.to_string().contains("test-symbol limit"));
    }

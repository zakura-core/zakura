...
| State-writer crash harness | `FaultPoint::ALL` and `FaultPoint::NO_CHANGE` durable/memory/publication handoffs | LC-TXN-01, LC-FRONTIER-04, LC-RECOVER-02 | durable AUD-14 | Partial: state-changing and no-change paired transactions are covered; every remaining transition variant and external response/reactor boundaries remain open |

...
The server MUST reserve serving capacity for bounded paths regardless of target storage. Ordinary transfers MUST NOT consume that reserve. A reserved lease admits at most 4,000 headers. Each header requires at most 2,048 encoded bytes, including the largest supported metadata and a complete response frame. The header cap therefore keeps the total response below 8 MiB; the byte allowance is not a separate admission check. An empty path requires one frame allowance. The wire codec tests MUST verify this allowance for each supported header format and auxiliary schema. Reserved leases expire 30 seconds after acquisition, even if the requester keeps paging. Continuations MUST NOT extend the admitted range. Leases admitted to ordinary capacity retain their renewable idle deadline.

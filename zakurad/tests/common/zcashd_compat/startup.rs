...
use super::{
    config::{expected_zakurad_chain_name, expected_zcashd_chain_name},
...
pub async fn both_processes_start() -> Result<()> {
    ...
    let expected_zakurad_chain = expected_zakurad_chain_name(&setup.network);
    ...
    assert_eq!(
        ...
        Some(expected_zakurad_chain.as_str()),
        "zakurad chain mismatch: expected {expected_zakurad_chain:?}, got {:?}",
        zebra_info["chain"]
    );
...
pub async fn miner_rpcs_disabled() -> Result<()> {
    ...
    if setup.can_mutate() {
        ...
        assert!(
            ...
            "zakurad getblocktemplate should return a template: {template}"
        );
    }

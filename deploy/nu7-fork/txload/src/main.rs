...
fn network_from_config(path: &std::path::Path) -> Result<Network> {
    ...
    let value: toml::Value =
        toml::from_str(&text).wrap_err("the node config is not valid TOML")?;
    let section = value
        .get("network")
...
async fn newest_spendable_utxo(
    ...
    let maturity = u32::from(transparent::MIN_TRANSPARENT_COINBASE_MATURITY);
...
fn prove_shielding_bundle(
    pk: &ProvingKey,
    value: u64,
) -> Result<orchard::bundle::Bundle<orchard::builder::InProgress<orchard::circuit::Proof, orchard::builder::Unauthorized>, i64>>
{
    let mut rng = rand_10::rng();

    // A throwaway recipient: the fork's shielded value is never spent again, and a note
    // nobody can decrypt is enough to move value out of the transparent pool.
    let sk = SpendingKey::from_bytes([17; 32])
        .into_option()
...
fn shielded_data_from_bundle(
    ...
    let flags = zorchard::Flags::from_bits(
        bundle
            .flags()
            .to_byte(bundle_version())
            .ok_or_else(|| eyre!("the bundle flags are not representable in this bundle version"))?,
    )
    ...
    Ok(zorchard::ShieldedData {
        ...
        binding_sig: reddsa::Signature::from(<[u8; 64]>::from(bundle.authorization().binding_signature())),
    })
}
...
async fn main() -> Result<()> {
    ...
    let secret = secp256k1::SecretKey::from_slice(
        &hex::decode(args.secret_key.trim()).wrap_err("the secret key is not hex")?,
    )
    ...
    loop {
        ...
        let shielded_value = u64::try_from(i64::from(utxo.value))
        ...
        let proved = prove_shielding_bundle(&pk, shielded_value)?;
        ...
        let transparent_sighash: [u8; 32] = draft_hasher
            .sighash(HashType::ALL, Some((0, utxo.script.as_raw_bytes().to_vec())))
            .0;

...
Duplicate padding can preserve the transaction Merkle root. The verifier MUST classify a duplicate list as a supplier payload mismatch unless removing its duplicates changes the root or the NU5 authorization commitment proves that the header commits to the duplicates. The verifier MUST check the transaction Merkle root before attributing a transaction branch-ID failure to the header. Semantic verification authenticates V5+ authorization data before dispatching transaction proofs. Duplicate attribution MUST reuse that parent context when available. If the parent context is unavailable, including while checkpoint verification queues its parent, a proven padding delivery remains a supplier fault and MUST NOT disqualify the header. Ordinary deliveries with missing context remain retryable under LC-BODY-03.

Each payload mismatch MUST advance the body's supplier retry episode. Native sync MUST defer the failed supplier before refetching and MUST exclude deferred suppliers from floor-server preference. The engine MUST publish the episode as body-unavailable evidence under LC-AVAIL-01 and LC-AVAIL-02, including the persistent alarm when the episode reaches its bound. This evidence MUST NOT disqualify the header or its descendants. The verifier does not reconstruct or validate a de-duplicated candidate.

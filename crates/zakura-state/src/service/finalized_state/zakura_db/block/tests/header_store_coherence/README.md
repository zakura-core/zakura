...
`zakura_db/block.rs`. Every reader assumes, but nothing enforces, that:
...
cargo test -p zakura-state --lib header_store_coherence
...
`zakura-state/proptest-regressions/` pins the seeds as a backstop and is
checked in.

...
/// Read a contiguous prefix without retaining more encoded block bytes than the caller permits.
///
/// The lookup for the first block that does not fit can temporarily materialize that one block,
/// but it is dropped immediately and never becomes part of the returned state response.

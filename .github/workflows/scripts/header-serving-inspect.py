...
for line in subprocess.check_output(['ps','-eo','pid,comm,pcpu,rss,etime'],text=True).splitlines():
    if any(word in line for word in ['cargo','rustc','zakurad']):
...
for name in ['/root/out/seed-priming/summary.json','/root/out/paired/summary.json','/root/out/paired/events.jsonl']:
    p=Path(name)
    if p.exists():

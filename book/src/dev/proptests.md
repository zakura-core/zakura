# Randomised Property Testing in Zakura

Zakura uses the [proptest](https://docs.rs/proptest/) crate for randomised property testing.

Most types in `zakura-chain` have an `Arbitrary` implementation, which generates randomised test cases.
...
1. in `zakura-chain`: make the `Arbitrary` impl depend on `#[cfg(any(test, feature = "proptest-impl"))]`
2. in the other crate: add zakura-chain as a dev dependency: `zakura-chain = { path = "../zakura-chain", features = ["proptest-impl"] }`

...
impl ValueBalance<NonNegative> {
    ...
    ///
    /// The resulting [`ValueBalance`] has `MAX_MONEY / 8` in each populated pool, keeping the total
    /// within the valid [`Amount`] range while leaving headroom for test value pool changes.
    #[cfg(any(test, feature = "proptest-impl"))]

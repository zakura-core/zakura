...
use zakura_state::{ChainTipChange, LatestChainTip, MAX_BLOCK_REORG_HEIGHT};
use zakura_test::command::TestChild;

use crate::common::{
    launch::spawn_zakurad_for_rpc_with_opts,
...
///
/// If this is set to `false`, but the state upgrades finish after zakurad is synced,
/// incomplete upgrades will be written to the cached state.
///
/// If this is set to `true`, but the state upgrades finish before zakurad is synced,
...
pub type BoxStateService =
    BoxService<zakura_state::Request, zakura_state::Response, zakura_state::BoxError>;

/// Waits for the startup logs generated by the cached state version checks.
/// Returns the state version log message.
///
/// This function should be called immediately after launching `zakurad`.
#[tracing::instrument(skip(zakurad))]
pub fn wait_for_state_version_message<T>(zakurad: &mut TestChild<T>) -> Result<String> {
    tracing::info!(
        zakurad = ?zakurad.cmd,
        "launched zakurad, waiting for zakurad to open the state database..."
    );
...
pub fn wait_for_state_version_upgrade<T>(
    ...
    tracing::info!(
        ...
        "waiting for zakurad state upgrade..."
    );
    ...
    if state_version_message.contains("launching upgrade task") {
        ...
        let upgrade_messages = zakurad.expect_stdout_line_matches_all_unordered(&required_logs)?;
        ...
        tracing::info!(
            ...
            ?required_logs,
            ?upgrade_messages,
            "zakurad state has been upgraded"
        );
    } else {
...
pub async fn load_finalized_tip_height_from_state_directory(
    ...
    let (_read_state, db, _sender) =
        tokio::task::spawn_blocking(move || zakura_state::init_read_only(config, &network))
            .await
...
pub async fn raw_future_blocks(
    ...
    let (zakurad, zakura_rpc_address) =
        spawn_zakurad_for_rpc_with_opts(network.clone(), test_name, test_type, should_sync, false)?
    ...
    zakurad.wait_with_output()?;
    std::thread::sleep(Duration::from_secs(3));

    let zakurad_state_path = test_type
        .zakurad_state_path(test_name)
        .expect("already checked that there is a cached state path");

    let Height(finalized_tip_height) =
        load_finalized_tip_height_from_state_directory(network, zakurad_state_path.as_ref())
            .await?;

//! Count the response items and bytes a peer is still allowed to send.
//! Local scheduling and handler capacity do not restore spent response credit.

use thiserror::Error;

/// The item and byte limits for one response. Each message checks its own identity
/// and ending. For example, two blocks using all their byte allowance still need
/// a separate BlocksDone before the request can be retired.
///

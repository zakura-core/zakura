...
- **`zakura-pr-node.yml`** — ephemeral one-hour real-node test of a PR or ref: boots a droplet from the pre-baked image, attaches a chain-state snapshot clone (`tip`, `pre-checkpoint`, `sandblast`, or `genesis`), builds the branch incrementally, runs it, and posts a metrics summary as a PR comment. `pre-checkpoint` uses the oldest retained tip snapshot so the node syncs through the last checkpoint handoff (the VCT frontier write) — the standard check for a branch that advances the checkpoints, such as a release-state bundle PR; it fails the run outright if the state starts at or above the branch's max checkpoint, so a green run always means a real crossing. The droplet stays up for SSH inspection until the reaper removes it.

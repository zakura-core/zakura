...
use crate::common::{launch::ZakuradTestDirExt, test_type::TestType};
...
///
/// If `wait_for_zakurad_mempool` is `true`, wait for Zebra to activate its mempool.
/// If `wait_for_zakurad_tip` is `true`, also wait for Zebra to sync to the network chain tip.
#[tracing::instrument]
pub fn wait_for_zakurad_and_lightwalletd_sync<
...
>(
    ...
    zakura_rpc_address: SocketAddr,
    test_type: TestType,
    wait_for_zakurad_mempool: bool,
    wait_for_zakurad_tip: bool,
) -> Result<(TestChild<TempDir>, TestChild<P>)> {
    let is_zakurad_finished = AtomicBool::new(!wait_for_zakurad_tip);
    let is_lightwalletd_finished = AtomicBool::new(false);

    let is_zakurad_finished = &is_zakurad_finished;
    ...
    // Optionally waits until Zebra has synced to the tip, based on `wait_for_zakurad_tip`.
    //
    // `lightwalletd` syncs can take a long time,
    // so we need to check that `zakurad` has synced to the tip in parallel.
    ...
    let zakurad_wait_fn = || -> Result<_> {
        ...
        if wait_for_zakurad_mempool {
            tracing::info!(
                ?test_type,
                "waiting for zakurad to activate the mempool when it gets near the tip..."
            );
            zakurad_mut.expect_stdout_line_matches("activating mempool")?;
        }

        // When we are near the tip, this message is logged multiple times
        if wait_for_zakurad_tip {
            tracing::info!(?test_type, "waiting for zakurad to sync to the tip...");
            zakurad_mut.expect_stdout_line_matches(crate::common::sync::SYNC_FINISHED_REGEX)?;
        }

        // Tell the other thread that Zebra has finished
        is_zakurad_finished.store(true, Ordering::SeqCst);

        tracing::info!(
            ?test_type,
            "zakurad is waiting for lightwalletd to sync to the tip..."
        );
        while !is_lightwalletd_finished.load(Ordering::SeqCst) {
            ...
            if wait_for_zakurad_tip {
                ...
                zakurad_mut.expect_stdout_line_matches(crate::common::sync::SYNC_PROGRESS_REGEX)?;
            }
        }
    ...
    //
    // `zakurad` syncs can take a long time,
    ...
    let lightwalletd_wait_fn = || -> Result<_> {
        tracing::info!(
            ?test_type,
            "lightwalletd is waiting for zakurad to sync to the tip..."
        );
        while !is_zakurad_finished.load(Ordering::SeqCst) {
        ...
        while !are_zakurad_and_lightwalletd_tips_synced(zakura_rpc_address, lightwalletd_mut)? {
    ...
    std::thread::scope(|s| {
        ...
        let zakurad_thread = s.spawn(|| {
            let zakurad_result = zakurad_wait_fn();
            is_zakurad_finished.store(true, Ordering::SeqCst);

            zakurad_result.expect("test failed while waiting for zakurad to sync");
        });
    ...
    Ok((lightwalletd, zakurad))
}

/// Returns `Ok(true)` if zakurad and lightwalletd are both at the same height.
#[tracing::instrument]
pub fn are_zakurad_and_lightwalletd_tips_synced(
    zakura_rpc_address: SocketAddr,
    ...
    rt.block_on(async {
        ...
        // Get the block tip from zakurad
        ...
        if lightwalletd_tip_height != zakurad_tip_height {
            tracing::info!(
        ...
        Ok(lightwalletd_tip_height == zakurad_tip_height)
    })
}

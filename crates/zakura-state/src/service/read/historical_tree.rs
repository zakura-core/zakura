...
fn anchor_for(
    ...
    let (cached, published) = {
        ...
        (
            cache.cached_anchor_at_or_below(height),
            cache.artifact_anchor_at_or_below(height),
        )
    };

    // Take the higher of the two: a cache hit anywhere at or below `height` is not automatically
    // nearer. The grid exists to bound cold replay, and a low cache entry must not hide a published
    // one a few thousand blocks below the target.
    if published_is_nearer(
        cached.as_ref().map(|(anchor_height, _)| *anchor_height),
        published.as_ref().map(|(anchor_height, _)| *anchor_height),
    ) {
        // `published_is_nearer` is true only when a grid entry exists.
        let (anchor_height, frontiers) = published.expect("a nearer published anchor is present");
        ...
        // trust weight, so a bad entry is ignored and the derivation falls back to the cache or to
        // replaying further. Making it fatal would hand anyone who can supply a corrupt artifact a
        // denial of service over a node that is perfectly capable of answering without one.
...
mod tests {
    ...
    fn artifact_at(height: Height, frontiers: &DerivedFrontiers) -> Arc<FrontierArtifact> {
        Arc::new(FrontierArtifact {
            spacing: 1,
            last_checkpoint: Height(height.0 + 1),

...
This is the highest configured checkpoint for which state has every
...
State advances this value only after the header transaction that completes the
...
highest completed checkpoint to witness the root at that height. State may
...
struct HeaderRootAuthState {
    ...
    highest_completed_checkpoint: HighestCompletedCheckpoint,
}
```
...
`highest_completed_checkpoint.height`, so the highest promotable root is strictly below that
checkpoint. Neither value is inferred from a network request flag or a successful
response.
...
highest completed checkpoints. Header sync resumes at the first missing height above
...
range marked `finalized` does not by itself advance this checkpoint.
...
1. Add `HighestCompletedCheckpoint`.
2. Advance the highest completed checkpoint only after a durable bracket-closing
   header commit.
3. Initialize or rebase the highest completed checkpoint from the canonical header
   store.
...
7. Require the successor witness itself to be covered by that checkpoint.
...
- Restart reconstructs the same highest completed checkpoint from durable headers.
...
- A successor witness above the highest completed checkpoint confirms no root.

...
#[cfg_attr(
    ...
    component(inject = "init_tokio(zakurad::components::tokio::TokioComponent)")
)]
...
impl TracingEndpoint {
    ...
    pub fn new(config: &ZakuradConfig) -> Result<Self, FrameworkError> {
        if config.tracing.endpoint_addr.is_some() && !cfg!(feature = "filter-reload") {
            warn!(addr = ?config.tracing.endpoint_addr,
                  "unable to activate configured tracing filter endpoint: \
                   enable the 'filter-reload' feature when compiling zakurad",
            );
        }
...
To set the filter, POST the new filter string to /filter:
    ...
    curl -X POST localhost:3000/filter -d "zakurad=trace"
"#
            .to_string(),
        ),

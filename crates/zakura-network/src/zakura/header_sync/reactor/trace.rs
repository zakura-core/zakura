...
use super::super::{
    ...
    events::{
        ...
        HeaderSyncEvent, HeaderSyncMisbehavior, HeaderSyncOperationKind,
...
enum HeaderTraceEvent {
    ...
    #[serde(rename = "header_root_auth_diagnostics")]
    RootAuthDiagnostics {
        height: block::Height,
        completed_checkpoint_height: block::Height,
        best_header_tip: block::Height,
        root_auth_hole_heights: u32,
...
enum HeaderEventProjection {
    ...
    HeaderRootAuthStateChanged {
        #[serde(skip_serializing_if = "Option::is_none")]
        height: Option<block::Height>,
        #[serde(skip_serializing_if = "Option::is_none")]
        completed_checkpoint_height: Option<block::Height>,
    },
...
impl HeaderSyncReactor {
    ...
    /// Emit the root-authentication frontier snapshot, but only when it differs
    /// from the last emitted one — this runs after every reactor event, and the
    /// snapshot is unchanged for the overwhelming majority of them.
    pub(super) fn trace_root_auth_diagnostics(&mut self) {
        ...
        self.startup
            .trace
            .emit_event(|| HeaderTraceEvent::RootAuthDiagnostics {
                height: snapshot.authenticated_height,
                completed_checkpoint_height: snapshot.completed_checkpoint_height,
                best_header_tip: snapshot.best_header_tip,
                root_auth_hole_heights: snapshot.hole_heights,
                first_retained_root_height: snapshot.first_retained_root_height,
            });
    }
}
    ...
    fn from(event: &HeaderSyncEvent) -> Self {
        match event {
            ...
            HeaderSyncEvent::WireHeaders {
                ...
                peer: peer_label(&wire_request.peer),
                range_count: saturating_usize(entries.len()),
            },
            ...
            HeaderSyncEvent::HeaderRootAuthStateChanged(state) => {
                Self::HeaderRootAuthStateChanged {
                    height: state.map(|state| state.authenticated_height),
                    completed_checkpoint_height: state
                        .map(|state| state.completed_checkpoint_height),
                }
            }
            ...
            HeaderSyncEvent::HeaderRootAuthenticationFailed { operation, kind } => {
                Self::HeaderRootAuthenticationFailed {
                    peer: peer_label(&operation.wire_request.peer),
                    session_id: operation.wire_request.session_id,
                    request_id: operation.wire_request.request_id.get(),
                    reason: root_auth_failure_reason_label(*kind),
                }
            }
            HeaderSyncEvent::BestHeaderTipLoaded {
                ...
                reanchor_from: *reanchor_from,
                reanchor: reanchor_from.is_some(),
            },
...
impl From<&HeaderSyncAction> for HeaderActionProjection {
    fn from(action: &HeaderSyncAction) -> Self {
        match action {
            ...
            HeaderSyncAction::QueryBestHeaderTip { reanchor_from } => Self::QueryBestHeaderTip {
                reanchor_from: *reanchor_from,
                reanchor: reanchor_from.is_some(),
            },
...
fn root_auth_failure_reason_label(kind: HeaderRootAuthenticationFailureKind) -> &'static str {
    match kind {
        HeaderRootAuthenticationFailureKind::Stale => "stale",
        HeaderRootAuthenticationFailureKind::CanonicalMismatch { .. } => "canonical_mismatch",
...
mod tests {
    ...
    use crate::zakura::{
        ...
        HeaderRangeEntry, HeaderSyncServiceSummary,
    };
    ...
    fn operation_identity_events_preserve_exact_field_order() {
        ...
        let payload = HeaderRangePayload::new(vec![HeaderRangeEntry {
            height: block::Height(1),

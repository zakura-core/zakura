...
The timeline answers where elapsed time went. When live CPU capture covers a block, its CPU profile button opens a dedicated full-page interactive viewer pinned to that exact request. New sampling does not fill in historical blocks. The default interval includes finalization after the verifier response. New flame graphs show estimated user CPU milliseconds from recorded CPU-clock periods, with sample-count views available in the profile selector. Historical profiles retain sample-count units. CPU time is distinct from block elapsed time. Actual timestamps and thread IDs remain in the CPU JSON. Samples show user-space process activity during the block, including any concurrent background work. Waiting and kernel execution are outside this measurement. Parallel spans overlap, and neither view claims an exact allocation of CPU milliseconds to one block. See [the live CPU design](../../docs/designs/block-profile-cpu.md) for capture and coverage guarantees.
...
A persistent systemd job builds the exact pushed revision on the dedicated host with four build jobs, a four-core CPU limit, and a 20 GiB memory limit. The current services continue running during tests and compilation. Only after success does it stop the node, drain the collector, install both executables, and start the collector, web service, and node. It verifies the new run's main and instrumented commits through the local API. New blocks may take longer to arrive, and normal startup timing exclusions still apply. The command leaves the proxy, quotas, and node lifecycle timers alone. It refreshes the sampler service and scoped profiler log rotation timer.
...
Pruning starts at 85% and targets 75%. Optional detail stops at 90%. CPU captures have a 20% retained allocation, while the sampler keeps raw captures below 8 GB with space reserved for its active files. Raw stacks are private and excluded from default exports. Each raw/decoded file has a 256 MiB limit. Daily reports retain at most 90 files. Summary retention targets two million attempts and prunes in bounded batches, with additional pruning under disk pressure. This is a rolling store with no long-term retention guarantee. Old completed block detail, CPU profiles, raw captures, reports, and unused symbols can be discarded as space is needed. Active recording and decoding files stay protected. These are byte/resource limits, not promised retention days. A dedicated five-minute log rotation timer checks collector, sampler, and recorder logs, rotating at 8 MiB with two compressed backups without restarting collection.
...
On the dedicated host, install `perf` for the running kernel, Python 3.11 or later, logrotate, and the quota tools. Ubuntu minimal images may also need `linux-modules-extra-$(uname -r)` for `quota_v2`. Verify quota enforcement before installing the services. Use the normal deployment build and preserve debugging information for useful Rust symbols. The profiling process never changes `perf_event_paranoid`.
...
The sampler resolves the exact supervised `zakurad` PID, verifies its executable and process start identity, and matches a fresh profiling run. It verifies the exact systemd control group against `/proc/PID/cgroup` and uses per-CPU events filtered to that group, with an additional exact PID filter during decoding. This avoids an exited-thread polling storm in the host's perf version while including newly created workers. It rotates the continuously running recorder every second at 999 or 2,000 Hz (every ten seconds at lower rates) and decodes closed segments in a separate process. A node restart starts a new capture session automatically. No PID-name matching is used. Stop and disable the sampler to return to timeline-only collection.

For a bounded operator canary, invoke `sample.py --store STORE --pid PID --executable EXACT_PATH --frequency 999 --duration-seconds 120` in the same restricted service environment. A zero duration means continuous collection. The service no longer uses the old fixed-PID environment file or a one-day expiry. Capabilities remain restricted to the sampler unit. An unsupported kernel or denied perf access leaves timelines available. Decoding keeps symbolized stack frames without expanding compiler-inlined calls, avoiding expensive `addr2line` expansion.

The raw spool, symbol cache, import inbox, and decoded captures stay under the profiler's filesystem quota. New symbol generations have a separate 2 GB budget and a 1 GiB limit per executable. Cached executables are hash-verified before atomic publication, and interrupted copies are rebuilt. Obsolete generations can expire after their raw captures have been decoded. Existing legacy symbol snapshots remain under the shared quota. Perf record and decode children retain their 256 MiB file limits. Backlog and query limits are explicit in CPU coverage. Short blocks can have few or zero statistical samples. New profiles normally become available after rotation, decoding, and the collector's next import pass. Downloads preserve full symbols and exact monotonic sample timestamps, along with per-sample CPU-clock periods for new captures. The flame graph defaults to estimated CPU milliseconds and offers sample-count views. Hover shows Self (this function), Total (including calls), percentages, and sample counts. Time weights are estimates for retained samples, not exact per-block CPU ownership or elapsed-time reconstruction. The higher rate requires checking decoder backlog, losses, and resource use. A lower-rate rollback is available with `--frequency 99`.
...
## CPU stage attribution

New recordings store disjoint active-context intervals on Linux threads. Async context applies only while a future is polled. Synchronous scopes explicitly enter stage context, including state finalization. Nested context switches split the outer interval. Missing inner records therefore leave gaps rather than assigning their work to the parent. Execution detail is capped at 65,536 intervals per attempt, one million retained intervals globally, and stops being retained at 90% store utilization. Intervals are removed with their attempts or by oldest-first retention. Producer loss contributes to run health without changing elapsed-span completeness. Missing or expired associations leave CPU samples unassigned.

The CPU page defaults to recorded context followed by native frames. The CPU page stage selector includes that stage and its recorded descendants. Raw stacks preserve the original symbols. Unassigned CPU includes work without exclusive context, including shared verification batches and currently uninstrumented parallel iterations. No whole batch is charged to one transaction. Execution context is suppressed while Rayon can run unrelated stolen work. This is incremental instrumentation, not complete causal tracing of every task. The clock uncertainty margin is excluded at each association boundary. CPU-period weights remain statistical estimates, not exact per-stage accounting. Old recordings are never backfilled from completion-thread IDs.

The sampler accepts `--stack-bytes` values 8192, 16384, 32768 and 65528. The service trials 32768 at the existing 999 Hz. Capture metadata retains the setting. The CLI default remains 8192 for compatibility. Restore `--stack-bytes 8192` in the service and restart only the sampler to roll back capture depth. Larger captures increase raw traffic and decode work. Existing file, backlog, memory and storage bounds remain enforced. Extreme sustained CPU load can reach these bounds and report capture loss or restart the sampler. Check CPU status, collection loss, raw growth, and stack recovery after changing it. Do not infer zero profiling overhead from different live blocks.

The waterfall has no per-stage CPU links. The block-level flame graph remains available. New profiles show shared proof verification separately: one elapsed worker execution per batch with the participating transaction numbers. The same batch can serve other blocks or unprofiled work. Those requests are counted as other or unlinked, never silently allocated to this block. These times overlap transaction waits and must not be added to the block total. Membership spans do not extend the recorded block boundary. CPU samples are not assigned to a transaction or batch from an elapsed interval.
...
The sampler and collector also accept 2,000 Hz for a measured canary. Change both supported components before increasing the live rate. Compare capture loss, omitted samples, decoder lag, cgroup CPU and memory pressure against 999 Hz. A different set of live blocks cannot establish zero timing overhead. Keep the node running during a rate change and restart only the sampler.
